...
# 市場時機措辭：一個 Health & Fitness 分類的 app 不得在自己的畫面上敘述盤勢。
# 依據 ANTIGRAVITY.md §1.4（「市場時機指引」絕對禁止）與 docs/PLAYBOOK.md §279
# （內部／領域術語不上畫面）。
#
# 這條規則已經被違反過兩次：先是 MODE_2 撞上使用者的專業術語，再來是
# 「盤整迴避時段」直接出現在快訊 chip 與設定開關上（#227 修掉，躺了六週）。
# 兩次都沒有守門，所以第三次只是時間問題。
#
# 🔴 沿用本檔既有的哲學：擋**命名**，不擋**解釋**。
#    講明「為什麼這個字不能用」的註解必須留得住 —— 否則這條規則自己的說明
#    會被自己擋掉。做法是**先濾掉註解行**再掃。
# ⚠️ 邊界（誠實說出來）：只認行首的註解標記，寫在程式碼**後面**的行末註解掃得到。
#    那是刻意的取捨 —— 寧可偶爾誤報一次，不要讓「把字串藏進行末註解」變成繞道。
# ⚠️ 刻意**不**擋「美東」：時區不是盤勢宣稱。過寬的規則會被關掉，
#    而這條要活很久（PLAYBOOK：守門抓到不像違規的東西，先問是不是範圍太大）。
BANNED_MARKET_REGEX='盤整|盤勢|盤中|開盤|收盤|美股|大盤|港股'

# 濾掉行首是註解的新增行（`*` JSDoc 續行 / `//` / `<!--`）
ADDED_CODE=$(echo "$ADDED" | grep -Ev '^\+[[:space:]]*(\*|//|<!--)' || true)

MARKET_HITS=$(echo "$ADDED_CODE" | grep -nE "$BANNED_MARKET_REGEX" || true)

if [ -n "$MARKET_HITS" ]; then
  echo "🚫 新增的代碼在敘述市場狀態 — 畫面上不得出現盤勢措辭:"
  echo "$MARKET_HITS"
  echo ""
  echo "   規則出處: ANTIGRAVITY.md §1.4（市場時機指引絕對禁止）、"
  echo "             docs/PLAYBOOK.md §279（領域術語不上畫面）"
  echo "   註: 解釋這條規則的**註解**不受此限（本檢查會先濾掉註解行）。"
  echo "       標籤要講「這是什麼時段」，不要講「市場為什麼這樣」——"
  echo "       例：'盤整迴避時段' → '安靜時段'（#227）。"
  exit 1
fi
