...
const INSTRUCTION_RULES = [
    {
        ...
        applies: ({ readiness }) => readiness !== undefined &&
            !readiness.ready &&
            (readiness.blocker === 'no_contact' || readiness.blocker === 'no_signal'),
    },
    {
        instruction: 'cover_more',
        applies: ({ reading, readiness }) => (readiness !== undefined && !readiness.ready && readiness.blocker === 'partial_contact') ||
...
export const INSTRUCTION_PRIORITY = INSTRUCTION_RULES.map((r) => r.instruction);
/**
 ...
 */
export function resolveCaptureStage(input) {
    ...
    const decorate = (stage, yieldsReading) => ({
        stage,
    ...
    if (readiness !== undefined && !readiness.ready) {
        return decorate(readiness.blocker === 'no_signal' || readiness.blocker === 'no_contact'
            ? 'searching_contact'
            : 'locking_light', false);
    }
    if (lock.locked)
        return decorate('pulse_locked', wouldYieldReading);
    ...
    const regressed = instruction !== null &&
        input.previousStage !== undefined &&
        PAST_LIGHT_LOCK.includes(input.previousStage);
    if (regressed)
        return decorate('needs_adjustment', wouldYieldReading);
    const lightSettled = reading.lightStability >= LIGHT_SETTLED && (exposure === null || !exposure.slowDriftDominates);
    ...
    return decorate(reading.meetsReadingGate ? 'pulse_candidate' : 'searching_rhythm', wouldYieldReading);
}
/**
 ...
 */
export function assessCoverageEvidence(input) {
    const { reading, exposure, uncoveredCells } = input;
    const coverageConfirmed = uncoveredCells !== undefined && uncoveredCells !== null && uncoveredCells === 0;
    const lightUniform = reading.lightStability >= LIGHT_SETTLED && exposure?.slowDriftDominates !== true;
    const contactSteady = 1 - reading.motionArtifact >= STILLNESS_FOR_RHYTHM &&
        reading.contactCoverage >= CONTACT_FOR_LIGHT;
    return {
        coverageConfirmed,
        lightUniform,
 ...
 */
function resolveLensState(stage, instruction, evidence, reading) {
    switch (instruction) {
        case 'cover_lens':
    ...
    if (reading.rhythmicCoherence !== null)
        return 'rhythm_search';
    if (evidence.allConfirmed)
...
function pickInstruction(input) {
