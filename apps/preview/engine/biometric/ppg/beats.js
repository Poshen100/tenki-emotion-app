 ...
 */
import { median } from './filtering.js';
 ...
 */
export function toIntervals(peakTimesMs) {
    const intervals = [];
 ...
 */
export function intervalTimes(peakTimesMs) {
 ...
 */
export function rejectArtifacts(intervals, timesMs = []) {
    ...
    for (let i = 0; i < intervals.length; i++) {
        ...
        if (neighbours.length === 0) {
            ...
            if (timesMs[i] !== undefined)
                acceptedAtMs.push(timesMs[i]);
            continue;
        }
 ...
 */
export function computeRmssd(intervals, minIntervals = MIN_INTERVALS_FOR_HRV) {
 ...
 */
export function computeSdnn(intervals, minIntervals = MIN_INTERVALS_FOR_HRV) {
    if (intervals.length < minIntervals)
        return null;
 ...
 */
export function heartRateFromIntervals(intervals) {
    if (intervals.length === 0)
        return null;
