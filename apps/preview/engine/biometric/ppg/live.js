 ...
 */
export function assessLiveWindow(frames, mode) {
...
function frameLevelReasons(parts) {
    const reasons = [];
    if (parts.contactComponent < 0.5)
        reasons.push('unstable_coverage');
    else if (parts.coverage >= 0.95)
        reasons.push('full_coverage');
    if (parts.stability < 0.5)
        reasons.push('motion_detected');
    else if (parts.stability > 0.8)
        reasons.push('low_motion');
    if (parts.lightComponent < 0.5)
        reasons.push('sensor_clipping');
    return reasons;
}
export const INITIAL_PULSE_LOCK = { consecutive: 0, locked: false };
/**
 ...
 */
export function advancePulseLock(state, reading) {
    const holds = reading.meetsReadingGate && (reading.rhythmicCoherence ?? 0) >= MIN_LOCK_COHERENCE;
...
export function recentFrames(frames, seconds = LIVE_WINDOW_SEC) {
    if (frames.length === 0)
        return frames;
    ...
    while (start > 0 && frames[start - 1].timestampMs >= cutoff)
        start--;
