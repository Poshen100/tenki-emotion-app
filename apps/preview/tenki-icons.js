...
(function (root) {
  ...
  var ACC_LIT = 'var(--tenki-accent-lit, #9CFFEF)';
  var ACC_DIM = 'var(--tenki-accent-dim, #1C6F73)';

  // Unique gradient id per call — multiple instances on one page must NOT share
  // a <radialGradient>/<linearGradient> id, or they'd all reference the first.
  var _gid = 0;
  function nextId() { return 'tenki-g' + (++_gid); }
  ...
  // 指紋建立 — UXWing 指紋（CC0），徑向漸層上色（中心亮、外緣深）。
  // 自帶 viewBox 122.88×121.74；外層以 size 控制顯示大小。
  function fingerprint(size) {
    var id = nextId();
    return '<svg viewBox="0 0 122.88 121.74" fill="url(#' + id + ')" stroke="none"' +
      ' aria-hidden="true" width="' + size + '" height="' + size + '">' +
      '<defs><radialGradient id="' + id + '" cx="0.42" cy="0.38" r="0.72">' +
      '<stop offset="0" stop-color="' + ACC_LIT + '"/>' +
      '<stop offset="0.5" stop-color="' + ACCENT + '"/>' +
      '<stop offset="1" stop-color="' + ACC_DIM + '"/>' +
      '</radialGradient></defs>' +
  ...
  function soulFace(size) {
    ...
    return '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"' +
      ...
      '<stop offset="1" stop-color="' + ACC_DIM + '"/>' +
      '</linearGradient></defs>' +
      '<g stroke="url(#' + id + ')" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none">' +
      '<path d="M4 12 h3 l1.2 -4 l1.6 8 l1.3 -5 l1.1 3 h1.5 l1.2 -2.4 l1.4 4 l1 -3.6 h2.2"/>' +
      '<circle cx="12" cy="12" r="9" stroke-width="1" opacity="0.22"/>' +
      '</g>' +
      '<circle cx="12" cy="12" r="1" fill="url(#' + id + ')" stroke="none"/>' +
      '</svg>';
  }
