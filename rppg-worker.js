...
function fftReIm(re, im) {
  ...
  for (let j = 1; j < n - 1; j++) {
    ...
    i ^= bit;
    if (j < i) { [re[j], re[i]] = [re[i], re[j]]; [im[j], im[i]] = [im[i], im[j]]; }
  }
    ...
    for (let i = 0; i < n; i += len) {
      ...
      for (let j = 0; j < len / 2; j++) {
        ...
        const vRe = re[i + j + len / 2] * wRe - im[i + j + len / 2] * wIm,
          vIm = re[i + j + len / 2] * wIm + im[i + j + len / 2] * wRe;
...
function update() {
  ...
  for (let i = 0; i < gSig.length; i++) {
    const x = gSig[i];
    const y = 0.55 * biquadProcess(bp1, x) + 0.45 * biquadProcess(bp2, x);
    bp.push(y);
  }

  const res = gSig.map((x, i) => x - bp[i]);
  const vBP = variance(bp), vRes = variance(res), nSQI = vRes > 0 ? vBP / (vRes) : 0;

  const { freq: hrHz, snr: hrSnr } = pickPeakFreq(bp, fs, 0.7, 3.0);
  const hrBpm = hrHz ? hrHz * 60 : null;

  const env = bp.map(v => Math.abs(v));
  const rrBp = biquadBandpass(fs, 0.25, 0.8), rrArr = [];
  for (let i = 0; i < env.length; i++) rrArr.push(biquadProcess(rrBp, env[i]));
  const { freq: rrHz, snr: rrSnr } = pickPeakFreq(rrArr, fs, 0.1, 0.7);
  const rrBrpm = rrHz ? rrHz * 60 : null;

  // --- HRV (RMSSD) + usable IBI count + gate thresholds ---
  const peaks = detectPeaks(bp, ts, 45, 180);

  let rmssd = null;
  let nIbiUsable = 0;

  const ibisRaw = [];
  for (let i = 1; i < peaks.length; i++) ibisRaw.push(peaks[i] - peaks[i - 1]);

  const ibis = [];
  for (let i = 0; i < ibisRaw.length; i++) {
    const ibi = ibisRaw[i];
    if (ibi < 300 || ibi > 1500) continue; // 40~200 BPM
    if (i > 0) {
      const prev = ibisRaw[i - 1];
      if (prev >= 300 && prev <= 1500 && Math.abs(ibi - prev) > 350) continue;
    }
    ibis.push(ibi);
  }
  nIbiUsable = ibis.length;

  if (ibis.length >= 4) {
    const diffs = [];
    for (let i = 1; i < ibis.length; i++) diffs.push(ibis[i] - ibis[i - 1]);
    const msq = diffs.reduce((a, b) => a + b * b, 0) / diffs.length;
    rmssd = Math.sqrt(msq);
  }

  const scanMode = (meta && meta.scanMode) ? meta.scanMode : "lock"; // "lock" | "spectrum"
  const thrQ = (scanMode === "spectrum") ? 0.75 : 0.6;
  const thrIbi = (scanMode === "spectrum") ? 30 : 12;

  if (Number.isFinite(hrBpm)) { hrHistory.push(hrBpm); if (hrHistory.length > 10) hrHistory.shift(); }
  const hrStd = std(hrHistory);

  const in2 = Object.assign({}, meta, { nSQI, hrBpm: hrBpm || 0, hrStd });
  const { sqs, penalties } = computeSQS(in2);
  const grade = gradeFromSQS(sqs);
  ...
  const w = weightsByGrade(grade);
  let tei_raw = null, tei_pr = null;
  if (w && grade !== 'D') {
  ...
  const quality = clamp(sqs / 100, 0, 1);
  const gate = {
    scanMode,
    thrQ,
    thrIbi,
    qualityPass: quality >= thrQ,
    ibiPass: nIbiUsable >= thrIbi,
    pass: (quality >= thrQ) && (nIbiUsable >= thrIbi)
  };
  ...
  lastMetrics = {
    ...
    quality,
    gate,
    fs, nSQI, sqs, grade, penalties,
    pr: { hr_pr, hrv_pr, rr_pr, sq_pr, tei_pr },
    tei_raw,
    weights: w
  };
...
function capBuffers(maxN) {
  if (ts.length > maxN) {
    const cut = ts.length - maxN;
    ts.splice(0, cut);
    rgb.splice(0, cut);
  }
}
...
onmessage = (e) => {
  ...
  if (msg.type === 'start') {
    ...
    hrHistory = [];
    frameCount = 0;
    postMessage({ type: 'started', scanId });
  }
