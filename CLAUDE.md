...
| **畫面顯示一個合成數字，卻沒給齊它的組成** | 那個數字就是任意的。實測：品質分數有**六個**分量而畫面只有**四條** bar —— 灌流佔 25（並列最重）連 bar 都沒有，於是 54 分答不出為什麼（`docs/PHONE-PPG.md` §25）。理由不能取代數字：「脈搏訊號偏弱」講不出 5.6/25 |
| **解釋用的算術跟被解釋的東西各寫一份** | 它會漂成一個看起來更合理的第二套說法。共用同一個函式，並用「各列加起來等於那個分數」釘住（§25）|
| **只用理論值訂門檻，不算它實際會遇到的量** | 兩邊都要算。實測：安靜段下限 5 秒是從估計器推的（對的），但沒人算過裝置的平原實際剩幾秒（**5.2**）—— 門檻剛好卡在上面，救援永遠不觸發（`docs/PHONE-PPG.md` §24）|
| **忘記預處理視窗的代價落在別的地方** | 平滑窗是為了不把脈搏讀成 slew，但它同時從**每一段的兩端各扣一截** —— 那個副作用才是卡住實機的東西（§24）|
| **一個狀態被兩條路徑到達，卻只有一句文案** | 文案只能對其中一條。實測：`light_locking` 同時來自良性 fallback 與 `camera_adapting`，於是相機在自己調亮度時畫面說「光已經穿過來了，保持這個位置」，而旁邊證據列正顯示「光場均勻 ✗」（§24）|
| **對使用者改不了的事給「調整動作」的指引** | 那是假指引，而且把責任推給他。相機在自己調增益，換姿勢改不了 —— 要說的是發生什麼事＋那個真的按得到的動作在哪裡（§24）|
