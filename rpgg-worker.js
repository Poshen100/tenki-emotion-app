...
function findPeaks(x, fs) {
    ...
    const minDist = Math.floor(fs * 0.32);
    let peaks = [];
    let last = -1e9;
...
function rmssdFromPeaks(tMs, peaks) {
    if (peaks.length < 4) return { bpm: null, rmssd: null, ibi: [] };

    const ibi = [];
    for (let i = 1; i < peaks.length; i++) {
        ibi.push(tMs[peaks[i]] - tMs[peaks[i - 1]]);
    }
...
function compute(windowMs) {
    ...
    const { bpm, rmssd } = rmssdFromPeaks(t, peaks);
    const quality = qualityScore({ fs, peaks, rmssd, bpm, x });

    return { bpm, rmssd, quality, nPeaks: peaks.length };
}
...
self.onmessage = (e) => {
    ...
    if (msg.type === "reset") {
        bufT = [];
        bufX = [];
        lastMetrics = { bpm: null, rmssd: null, quality: 0, nPeaks: 0 };
    ...
    if (msg.type === "compute") {
        const res = compute(msg.windowMs || 15000);
        if (!res) return;
        lastMetrics = res;
