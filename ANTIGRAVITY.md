...
> Tier id 維持 `premium`（避免既有訂閱狀態 migration），顯示名稱為 Pro。

**掃描次數兩個 tier 都不限。** 差異由四條軸承擔：

| 軸 | Free | Pro |
|----|------|-----|
| **時間深度** | 7 天歷史 | 無限歷史 + 完整 Timeline |
| **分析深度** | 分數 + Zone + 每日一則基本觀察 (Coach P1) | Edge Graph、Coach P2–P4、Replay 進階 |
| **主動性** | Detector **靜音**運作 + 每日回顧 | Detector **即時**提醒 |
| **情境廣度** | Health Reset | 全部 Scenario Modes + Trader Templates |

Pro 另含：匿名 Benchmark、雙耳節拍、External alert bridge。

**兩條不可破的規則：**

1. **永遠不要為了製造差異而降低免費版分數的保真度。** 不削維度、不降 baseline 精度、
   不故意壓 confidence。閘門放在**衍生的智慧**，不放在**量測的保真度**——
   病毒迴圈靠新安裝的人很快拿到**真實**數字，而準確度是健康產品的信任地基。
2. **把牆放在使用頻率最高的地方。** 歷史截斷與靜音提醒是每天碰到的，
   年度回顧一年碰一次——前者才有轉換力。
...
|------|------|
| 基本掃描能力 | 核心互動不能被鎖住 |
| **掃描次數（無限）** | 用掃描次數收費 = 向使用者收費請他們幫你蓋護城河（見 §13.2）。掃描是裝置端運算，邊際成本趨近零 |
| Edge Score 計算 | 基本價值必須免費體驗 |
| **Edge Score 保真度** | 免費版用完整 8 維度 + 完整 baseline 引擎。刻意做爛的分數會教會使用者「這產品不準」 |
| 數據刪除 / 匯出 | 隱私權利永不付費 |
| 基本歷史 (7 天) | 最低限度的自我追蹤 |
| 多巴胺狀態日誌 | 自我覺察是基本功能 |
...
**額度牆已完全移除（§12.1，2026-08-13）**，掃描次數不再是轉換槓桿，
所以時機式觸發器現在是**唯一**的轉換引擎。觸發點是**資料剛好成熟到能解鎖某個洞察**的那一刻：
...
沒有額度牆之後，免費版必須**看得見自己被擋住的東西**——而且要看得見的是
**他自己的真實資料**，不是假的預覽圖。Edge Graph 對免費用戶要真的畫出來，
只是只有 7 天、其餘灰掉。真實的殘缺比漂亮的鎖頭圖轉換率高得多。
