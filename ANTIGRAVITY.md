...
2. **Mobile 日常掃描揭曉的實機手感**：Expo 實走 Scan tab → 日常 ceremony → `/scan/result` 揭曉 → Today；揭曉動效/節奏 polish（Reanimated 3；環形圖 Skia，禁 SVG）。注意：established 畫面文案是「首次基線」導向，日常掃描已繞過它 — 若 founder 想要日常專屬的揭曉儀式畫面，屬設計決策，先和 founder 對齊再動。
