...
export interface ValidationCapture {
  ...
  /**
   * Whether the torch was actually lit for this capture, or null on a browser
   * that has no torch to light (every iOS Safari).
   *
   * 🔴 Recorded so the device can answer a question I would otherwise be
   * guessing at. The drift is 29% of the level and the browser will not lock
   * exposure, so the remaining lever is to stop giving auto-exposure something
   * to hunt — and a torch pressed against a fingertip puts red at 201 while
   * green sits at 37, which is an extreme scene for an AE loop to balance.
   * Whether the torch *causes* the hunting is a two-capture experiment, not an
   * argument, and splitting the drift by this field is what makes it one.
   */
  torchOn: boolean | null;
  /**
   * What the quiet-stretch rescue found, or null when it was never attempted.
   *
   * 🔴 Recorded including its refusals. The rescue failing is not one fact but
   * three — too few stretches, stretches that carried no usable pulse, or
   * stretches that disagreed — and they point at three different repairs.
   * Without this the device can only report that the rhythm was 0 again.
   */
  quietSegments: {
    bpm: number | null;
    foundCount: number;
 ...
 */
function quietSegmentNote(log: readonly ValidationCapture[]): string {
  const tried = log.filter(
    (c): c is ValidationCapture & { quietSegments: NonNullable<ValidationCapture['quietSegments']> } =>
      present(c.quietSegments),
  );
  if (tried.length === 0) return '安靜段讀數 沒有任何擷取走到這一步（節律不是唯一被擋的理由）';
  ...
  const notPeriodic = tried.filter(
    (c) => c.quietSegments.foundCount >= 3 && c.quietSegments.periodicCount < 3,
  );
 ...
 */
function torchSplitNote(
  exposures: readonly (ValidationCapture & { exposure: ExposureStability })[],
): string {
