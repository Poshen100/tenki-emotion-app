...
import { bandPass, detrend, dominantPeriod } from './filtering';
...
/** A respiration estimate and how strongly the evidence supported it. */
export interface RespirationEstimate {
  brpm: number;
  /** Dominant-component strength inside the respiratory band, 0..1. */
  periodicity: number;
}
...
export function estimateRespiration(
  ...
  if (spanSec < MIN_TACHOGRAM_SEC) return null;
 ...
 */
function agreesAcrossHalves(band: readonly number[], fullLag: number, spanSec: number): boolean {
  if (spanSec < MIN_TACHOGRAM_SEC * 2) return true;
  ...
  for (const half of [band.slice(0, mid), band.slice(mid)]) {
    ...
    if (Math.abs(found.lagSamples - fullLag) / fullLag > HALF_AGREEMENT_TOLERANCE) return false;
  }
