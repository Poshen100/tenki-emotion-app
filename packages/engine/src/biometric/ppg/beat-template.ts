 ...
 */
export function beatTemplateCorrelation(
  values: readonly number[],
  periodSamples: number,
  peakIndices: readonly number[],
): number | null {
  const length = Math.round(periodSamples);
  if (length < 4 || peakIndices.length < MIN_BEATS_FOR_TEMPLATE) return null;
  ...
  for (const peak of peakIndices) {
    ...
    if (start < 0 || start + length >= values.length) continue;
    segments.push(values.slice(start, start + length));
  }
  if (segments.length < MIN_BEATS_FOR_TEMPLATE) return null;

  const template = Array.from(
    { length },
    (_, i) => segments.reduce((sum, seg) => sum + seg[i], 0) / segments.length,
  );
