...
/** Accumulated repeatability measurements for one user. */
export interface NoiseFloorState {
  /** Most recent per-scan repeatability values, oldest first, in ms. */
  samples: number[];
}

/** An empty state, for a user who has never completed an HRV scan. */
export function createEmptyNoiseFloor(): NoiseFloorState {
  return { samples: [] };
}
 ...
 */
export function recordRepeatability(
  ...
  return {
    samples:
      samples.length > NOISE_FLOOR_WINDOW ? samples.slice(samples.length - NOISE_FLOOR_WINDOW) : samples,
  };
}
 ...
 */
export function resolveNoiseFloor(state: NoiseFloorState): number | null {
 ...
 */
export function resolveEffectiveStd(baselineStd: number, noiseFloorMs: number | null): number {
  if (noiseFloorMs === null) return baselineStd;
