...
| 量測中 | 飽和度 **0.70–1.35**、漸層寬度 1.0→0.45、粒子漂移 ×1.35→×0.55、亮度 ±0.06；色相 **0 → +0.06 turn 單向**（**越穩越回到 0**＝回到身分） | `stillness`（**實測**，而且正是畫面上要求使用者控制的那個量） |
| 量測中（累積） | 往 `cyanActive` 聚焦 ≤0.30 + 尺度收緊 6% | `heldMs/budgetMs` —— **只在閘門通過時前進**，是累積的有效量測，不是計時器 |
...
🔴 **飽和度下限 0.70 與 focus 上限 0.30 都是 ΔE 掃描定出來的，不是手感挑的**：
「去飽和的青」與「收向青」都會逼近 `--zone-neutral #64748B`（代表 Neutral 帶位）——
0.55/0.55 時 ΔE 只有 17.0。現行組合最小 ΔE 27.3，限制條件是 coral 而非 neutral。
