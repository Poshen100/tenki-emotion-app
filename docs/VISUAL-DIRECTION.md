...
| 時刻 | 顏色與形態 | 依據 |
|------|------|------|
| 靜息 | cyan→purple→pink 原樣（彩度跨度 90） | — |
| 量測中·晃動 | 完整基礎漸層、尺度 ×1.18、暗 | `stillness` 低 |
| 量測中·穩住 | **色相散開，彩度跨度衝到 172**、尺度 ×0.86、亮 | `stillness` 高 |
...
🔴 **顏色只會變多，不會變少。** 彩度跨度全程 **84–172**（靜息 90）。
...
而灰就是 `--zone-neutral`（Neutral 帶位）**。所以 bloom ≤ 0.20、旋轉 ≤ 0.20 turn：
bloom 0.30 → 主色 ΔE 19.3 ❌；旋轉 0.40 → 撞 `zone-strain` ΔE 3.0 ❌。
現行組合主色 ΔE **27.4** ✅。
