...
| 量測中 | 色相 ±0.10 turn 緩慢位移、飽和度隨穩定度起伏 | 眉間張力 / 嘴開合 / 位移穩定度（**實測 landmark 幾何**） |
| SECURED 那一拍 | `goldSecured` | 跟光弧同一拍。**沒有讀數就不准上 gold** |
| 收束停留 | `--zone-clear` / `--zone-neutral` / `--zone-strain` | 該次帶位 |

⚠️ 量測中的位移刻意小 —— 幅度大到跑進金色，就等於在還沒有結果時宣稱 SECURED。
