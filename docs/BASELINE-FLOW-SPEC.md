...
> **Status**: Draft v1.0 — Spec only, no code yet
...
| 用戶點右側 `(o≡)` 圖示 | **永遠**可以拉出 debug drawer(三條品質條 + 相機縮圖 + raw values)— 給好奇/開發者 |

### 2.3 狀態文案對照(替代「穩定中...」)

| 條件 | 文案(zh-Hant) | 顏色 token |
|---|---|---|
...
- 預設**隱藏**(極限主義)
- 訊號 red 時自動淡入右下角(96×96,圓角 16,陰影輕)
...
- Drawer 用 Reanimated 3 spring 從右側 `(o≡)` 滑出,不擋圓環
...
## 5. Open Questions(等 founder 回)

1. drawer 圖示要不要常駐?還是只在 amber/red 時才出現?(影響極限感)
2. 相機縮圖預設要不要在掃描全程都顯示一個極小版(8×8 angular dot)讓用戶知道相機運作中?
3. 「正在學習你的節奏」這句要不要保留為主畫面?還是依狀態換句子?
4. drawer 內三條品質條是否該加歷史 sparkline(過去 7 次掃描品質趨勢)?
