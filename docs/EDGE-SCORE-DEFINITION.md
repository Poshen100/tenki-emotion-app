...
🔴 **但畫面上只顯示一個計數。** 因為 `appendSample()` 同 profile 同一天後寫覆蓋，
`sampleCount` **恆等於** `distinctDays`（`scripts/preview-strip-color.mjs` 有一條
跨 9 天 × 每天 3 掃的斷言釘住這個恆等式）。所以環心寫「基線 n/30 · d/7 天」
是把同一個數字講兩遍 —— 它不但沒有新資訊，還會讓人以為有兩個獨立的門檻，
而且正是 2026-08-13 撐爆環心那行字的那一段。**顯示只留 n/30，閘門保留為縱深防禦。**
⚠️ 哪天有人拿掉「一天一筆」的去重，`基線 n/30` 就會開始誤導（n=30 但只跨 2 天）——
上面那條恆等式斷言會先紅，紅了就要把顯示改回兩個計數，不是把斷言刪掉。
