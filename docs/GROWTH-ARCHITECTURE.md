...
> **最後更新**：2026-08-13
> **版本**：v1.1（定價定案：無限掃描 + Free/Pro $9.99）
> **狀態**：Active
...
| 不得付費牆的功能 | `ANTIGRAVITY.md` §12.2 | 基本掃描、**掃描次數**、Edge Score、**分數保真度**、刪除/匯出、7 天歷史、多巴胺日誌、Edge Snapshot |
...
**Edge Snapshot 在 Free 與 Pro 兩個 tier 都必須開放。**
...
| 階段 | 能力 | 運算位置 | Tier | 依賴 |
|------|------|----------|------|------|
| **P1 規則式** | 時段 pattern、趨勢方向、zone 分布 | 裝置端 | Free（基本）/ Pro（完整） | 既有 `insight-generator.ts` |
| **P2 相關性** | 睡眠 × clarity、stress × clarity 的**個人**相關性 | 裝置端 | Pro | DPD 標註 ≥ 30 筆 |
| **P3 個人模型** | 個人化 pattern 模型 + 情境提示 | 裝置端 | Pro | DPD 標註 ≥ 90 筆，跨 3 時段 |
| **P4 語言層** | 自然語言解釋與問答 | 雲端（opt-in） | Pro | 明確 opt-in + 特徵去識別化 |
...
### 7.1 Tier 對照（2026-08-13 定案）

以 `packages/shared/src/subscription-tiers.ts` 為準。

> **Free $0 — 認識你現在的狀態。**
> **Pro $9.99/月・$89.99/年 — 認識你運作的方式。**
>
> 掃描是**現在式**，該送；會複利的東西（歷史、模式、相關性、比較、主動提醒）才收錢。

#### 為什麼掃描不限次數

這不是慷慨，是護城河的方向問題。每一次免費掃描都在：

1. 加深該使用者自己的 baseline —— 拉高**他自己**的轉換成本（§3.3 Layer 1）
2. 產生一筆潛在的 label —— 餵養最深的那層 moat（§3.3 Layer 3）
3. 在 opt-in 時推 cohort 靠近 k=50 —— 餵養網路效應（§4）

**用掃描次數收費 = 向使用者收費，請他們幫你蓋護城河。方向是反的。**

經濟上也撐得住：掃描是**裝置端**運算（PPG + 本地引擎），邊際伺服器成本趨近零。
唯一有雲端成本的是 Coach P4 與 benchmark 聚合，兩者都是 Pro 專屬 ——
所以免費用戶不是成本中心，是純燃料。這點與 LLM 型產品完全不同：
那類產品的免費額度真的在燒錢，TENKI 不是。

#### 四條差異軸

| 軸 | Free | Pro |
|----|------|-----|
| **時間深度** | 7 天歷史 | 無限 + 完整 Timeline |
| **分析深度** | 分數 + Zone + 每日一則基本觀察 | Edge Graph、Coach P2–P4、Replay 進階 |
| **主動性** | 你去找它（pull）—— Detector **靜音**跑 + 每日回顧 | 它來找你（push）—— Detector **即時**提醒 |
| **情境廣度** | Health Reset | 全部 Scenario Modes + Trader Templates |

第三軸是轉換力最強的一條。免費版讓 Detector 照常運作但靜音，晚上給一則回顧：

```
今天你的 clear window 出現在 09:40–11:15。
Pro 會在它開啟的當下就通知你。
```

它展示的是**已經發生在使用者自己身上**的價值，不是功能清單。誠實、具體，而且每天發生一次。
...
| 多巴胺狀態日誌 | ✅ | ✅ |
| 呼吸練習（4-7-8、方框） | ✅ | ✅ |
| Edge Detector | 靜音 + 每日回顧 | **即時提醒** |
| Scenario Modes | Health Reset | **全部** + Trader Templates |
| Personal Edge Graph | — | ✅ |
| Edge Timeline | 7 天 | 完整 |
| Decision Replay 洞察 | 基本 | 進階 |
| AI Coach | P1 基本 | P1–P4 完整 |
| Edge Prediction（內部代號） | — | ✅ |
| 匿名 Benchmark 比較 | — | ✅ |
| 雙耳節拍 | — | ✅ |
| External alert bridge | — | ✅ |

#### 不可破的規則

**永遠不要為了製造差異而降低免費版分數的保真度。**

不削維度、不降 baseline 精度、不故意壓 confidence。三個理由：

1. 病毒迴圈（§5.4）靠新安裝的人**很快拿到真實的數字**
2. 刻意做爛的免費分數會教會使用者「這產品不準」
3. 準確度是健康類產品的信任地基

**閘門放在「衍生的智慧」，不要放在「量測的保真度」。**

### 7.2 轉換觸發點：時機式，不是額度式

多數 app 用額度牆逼轉換（「今日次數已用完」）。TENKI **完全沒有額度牆**（§7.1），
所以時機式觸發是**唯一**的轉換引擎：在使用者的資料**剛好成熟到能解鎖某個洞察**的那一刻提示。
...
**1. 免費版要看得見自己被擋住的東西 —— 而且要是他自己的真實資料。**

Edge Graph 對免費用戶要**真的畫出來**，只是只有 7 天、其餘灰掉。
真實的殘缺比漂亮的鎖頭圖轉換率高得多，因為前者證明產品已經為你算好了，
後者只是在描述一個你沒看過的功能。

**2. 把牆放在使用頻率最高的地方。**

歷史截斷與靜音提醒是**每天**碰到的；年度回顧一年碰一次。
同樣是「Pro 才有」，前者每天製造一次張力，後者一年一次 —— 只有前者有轉換力。
...
| 月繳 | **$9.99 USD** | 與 Fitbit Premium 同價位；該產品同樣是「免費量測 + 付費洞察」的結構 |
| 年繳 | **$89.99 USD**（約 25% off） | 見下方計算 |
| 試用 | 7 天，**在 baseline 成熟前不啟動** | 試用期若落在 `new` baseline 階段，使用者看不到 Pro 的真正價值 |

**年繳數字的來源**：$9.99 × 12 = $119.88。~20% 折扣會落在 $95.99 —— 數字難看。
$89.99 約當 25% off，錨點更強、又壓在 $90 心理關卡之下。
`BILLING_CADENCES.yearly.discount` 已同步改為 `Save ~25%`。

> **價格來源的真相**：程式碼裡的 `priceUsd` **只供顯示**。App Store Connect 與
> Google Play 才是使用者實際被收多少錢的權威來源。任何 entitlement 判斷都不得
> 讀這個常數，否則商店調價時會出現「程式碼說 $9.99、商店收 $12.99」的漂移。

> **試用時機建議**：把 7 天試用綁在「baseline 達到 `ready`」而非「安裝後第 N 天」。
> 這確保試用期展示的是成熟資料下的 Pro 體驗，而不是空圖表。
...
| 反模式 | 為什麼不做 |
|--------|-----------|
| **用掃描額度收費** | 等於向使用者收費請他們幫你蓋護城河，同時掐住 flywheel 的輸入端（§7.1） |
| **為製造差異而降低免費版分數保真度** | 病毒迴圈需要新用戶很快拿到**真實**數字；做爛的分數會教會使用者「這產品不準」（§7.1） |
...
| # | 問題 | 影響 | 狀態 |
|---|------|------|------|
| 1 | Free tier 每日掃描次數：`ANTIGRAVITY.md` §12.1 寫 3 次，`subscription-tiers.ts` 寫 `dailyScanLimit: 1` | 文件與程式碼不一致 | ✅ **已解決 2026-08-13** — Founder 定案：**兩個 tier 都不限次數**，不做 A/B。理由是 A/B 問錯了問題——真正的問題不是「幾次」，而是「掃描次數該不該是付費槓桿」，而答案是不該（§7.1）。差異改由四條軸承擔，Pro 定價 $9.99/月 |
...
*— END OF GROWTH ARCHITECTURE v1.1 —*
