...
- **「我加了一條 harness 守住 X」在跑過反向驗證之前只是願望**（2026-08-08）。新增的每一條斷言，
  ...
  不要假裝在容器裡驗證了行為本身。
- **🔴 「閒置狀態」不得渲染「執行中狀態」的構造**（2026-08-12）。
  決策底座在 idle 時仍畫著事件點與一個虛線 `+`，而 `logEvent()` 在非 running 時
  **直接 return** —— 一個按了沒反應的控制項。founder 實走的第一句話是
  「好像是作一半的狀態」。
  它同時是**版面成本**問題：那組構造撐著 `--fdcb-h:58px`，而每一屏都保留
  `tabbar + 58 + gap`，於是**閒置的東西用執行中的尺寸付 5 個分頁的永久租金**，
  Snapshot 的線圖與 Timeline 的 Event Log 都被它切掉。
  規則：**元件的尺寸與出現範圍要跟著狀態走**；閒置＝入口（小、只在會用到的地方），
  執行中＝現場 HUD（大、跟著使用者跨頁）。

- **🔴 元件手上已經有的資訊就要用**（2026-08-12）。
  同一個底座在 idle 時早就算出 `segEl.textContent = '尚無讀數'`，**卻仍然在旁邊
  寫「START DECISION」** —— App 同時在說「我不知道你的狀態」和「去做決策吧」，
  牴觸 `SYSTEM.md` 的核心命題（先校準，再決策）。
  判斷不是「要不要多做一個檢查」，而是「這個元件已經知道的事，有沒有反映在它
  說的話裡」。

- **🔴 三種「其實驗不到東西」的斷言**（2026-08-12 一天內各踩一次，已成類別）。
  共同特徵：**全綠，而且看起來正中要害**。寫完每一條都要問「什麼樣的壞法會讓它紅」。

  | 型態 | 實例 | 判準 |
  |---|---|---|
  | **同義反覆** | 驗「t 與 t+60000 產出相同值」，但相位就是用 `t % 60000` 算的 → 等式**恆成立**，連壞掉的乘數 0.23 都通過 | 斷言的兩邊若同源，它證明的是自己 |
  | **值對但看不見** | 基線進度環被 `.tl-edge svg{display:none}`（給另一個 legacy 節點寫的規則）整個關掉，而 `stroke-dashoffset` 與文字**全部正確** | 凡是「使用者要看到」的東西，**必須有一條量幾何**（`getBoundingClientRect` / `elementFromPoint`），不能只驗值 |
  | **對照組沒跑到** | `git stash` 後跑截圖腳本，腳本因為找不到新元素而拋錯、沒產圖，我複製了上一輪的舊檔當「對照」→ 等於拿檔案跟自己比 | 對照組的腳本**不得引用待驗功能的任何元素**；產物要驗它真的是這一輪產生的 |
  | **驗到別人的功勞** | 「閒置不得渲染進度軌」全綠 —— 但把我新加的規則刪掉**它仍然綠**，因為早就有一條 `.fdcb.state-idle .fdcb-prog{display:none}`。我的分析從一開始就判斷錯了問題在哪 | 反向驗證要**刪掉自己那一份**，不是弄壞整個行為；還綠就代表這條守的不是你以為的東西 |
  | **把「要捲動」當成「被遮擋」** | 700px 高時 `elementFromPoint` 對輪播圓點回 null → 判定被底座蓋住。實際上那一段高度的頁面**本來就設計成會捲**，而視窗外的點本來就回 null | 量遮擋前先捲到底；驗的是**可及性**（捲得到且沒被蓋住），不是「一屏內看得到」 |
| **沙盒的字體不是裝置的字體** | 環心那行 `… · 基線 2/30 · 2/7 天` 在容器裡量到 172px / 可用 186px，**沒換行**；founder 的 iPhone 上換行了，「天」變成孤字。差別是容器沒有 PingFang TC，CJK 被代換成較窄的字 | 版面寬度**不要量像素**（`getBoundingClientRect` 在沙盒是另一種字體的答案）。改量與字體無關的量：**字元預算**（CJK/全形算 2 欄），上限由容器寬 ÷ 字級推導再留餘裕 |

  ⚠️ 補一條量測時序：`elementFromPoint` 在開場動畫還沒退場時回的是 `tenki-splash`，
  會把「被開場蓋住」誤判成「被底座蓋住」，然後往錯的方向修。**等待要夠久**（v6 約 4s）。

  ⚠️ 還有一條穩定性：取樣週期性訊號驗「有在動」時，**取樣窗要跨過一整個轉折**。
  正弦在波峰附近二階平緩，窗太短會偶爾讀到相同值 —— **偶爾紅的斷言比沒有還糟**。
...
- **🔴 進度條沒綁在「有效量測」上，它就是在說謊 —— 而且「算了但沒接」最難發現**（2026-08-11）。
  `/v3/` 的旗艦掃描儀式跑的是 `progress += dt / 8000`：手指按住的牆鐘時間。
  它**有**真相機、**有**真 FaceMesh、**有**算對位閘門（`getAlignmentHint`）——
  但 `updateLoop` 從來沒讀過 `align.aligned`。拒絕相機、沒有臉、手機蓋在桌上按住
  8 秒，一樣跑到 100%，而且什麼都不寫。**畫面看起來完全正常**，founder 實走了好幾輪
  才問出「準度跟另一支一樣嗎」。
  規則：**看到一個進度/百分比，先追它的加法在哪一行，以及那一行的條件是什麼。**
  「這個模組有開相機」不是證據；`grep` 那個閘門變數有沒有被第二個地方讀到才是。
  推論：同一個檔案裡出現「算得很認真的判定」與「沒有引用它的消費者」，
  就是這個 bug 的形狀 —— 比完全沒算更難抓，因為它讀起來像做完了。
- **🔴 環境缺一半時，只驗 UI 狀態的斷言會讓「把門改成永遠放行」全綠**（2026-08-11）。
  我加了 ceremony 的「按住才前進」，並寫了 6 條斷言（按住層有沒有出現、有沒有
  holding class、膠囊講什麼）。反向驗證把 `holdSatisfied()` 改成 `return true`
  —— 也就是完全退回上面那個純計時器的病 —— **6 條全綠**。
  原因是 harness 環境沒有相機，`tick()` 的取樣段根本不會跑，進度的耦合看不到。
  正解：**把那道門 export 出來直接驗**（純函式、無副作用），
  跟先前 `toneMatrix` / `hueBandOf` / `toneIdle` 同一個做法。
  判準：**斷言要能在「這個環境跑得到的東西」上失敗**，否則它只是註解。
