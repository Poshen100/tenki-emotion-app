...
- 🔴 **斷言要守 bug 的「家族」，不要守它這一次的「實例」。**
  2026-09-07 實例：「時鐘不准是紫的」只擋得住那一個元素；改寫成
  **「換一個模板 → 整屏顏色逐項不變（白名單除外）」**之後，同一條斷言
  **當場又抓到四個我漏掉的元素**（sparkline、來源切換鈕、輪播圓點…）。
  判準：問「這個 bug 的**形狀**是什麼」而不是「這次壞的是誰」——
  形狀通常是「某個會變的東西被綁到不該變的地方」，那就去斷言「它不該變」。
  ⚠️ 配套：**白名單要逐一列舉 id 並自己也有斷言**（那個例外必須真的還成立），
  否則白名單會慢慢變成一張放行整族的空頭支票。
- 🔴 **一個顏色可以完全可讀、完全穩定，而且在說謊 —— 那需要第三種斷言。**
  同一天實例：信心膠囊 `#E8B45A` 與 `--gold-secured` ΔE 12.2（gold ＝ SECURED），
  但它講的是「精度**還不夠**」。對比斷言過（10.67:1）、換模板斷言過（不吃 `--primary`），
  **只有「沒在講 SECURED 的東西不准長得像 gold」這條抓得到**。
  CLAUDE.md 的「顏色也會宣稱事實」要有機器版本，就是這個形狀：
  **語義色 + ΔE 門檻 + 逐一列舉的允許名單**。
- ⚠️ **量一個有 `transition` 的屬性，`getComputedStyle` 會給你過渡中的當下值。**
  同一天踩到：加完 class 立刻讀 `.tl-edge-conf`（`transition:all 0.6s`）的 color，
  拿回過渡**起點**的值，看起來像「我的 CSS 沒生效」，其實是我讀太早。
  改動 class 之後要等過渡跑完再讀（或讀 `getComputedStyle` 之前先關掉 transition）。
- 🔴 **runtime 掃描只看得到「此刻畫面上真的存在」的元素 —— 條件狀態一律看不到。**
  2026-09-08 實例：把一個退場的顏色放回 `.result.no_trade` 反向驗證，畫面掃描**照樣全綠**
  —— 因為那個 class **從來沒有被套用過**（產生它的模組只吐另外三個值，是一條死 CSS）。
  要涵蓋沒被渲染的狀態，就得掃 **`document.styleSheets` 本身**：每一條規則裡宣告的
  每一個值，不管它此刻有沒有匹配到元素。⚠️ 而且判準要用**數值**（ΔE / 對比），
  不要用字面 —— 字面比對換個寫法（`#F5A623` → `rgb(245,166,35)`）就靜默失效。
  ⚠️ 樣式表掃描自己的邊界：看不到 JS 寫的 inline style。兩種掃描互補，都要說出邊界。
- 🔴 **反面斷言（「這東西真的在」）最容易變成死斷言 —— 它必須跟正面斷言掃同一個範圍。**
  2026-09-08 實例：「這一面真的鋪了可動層嗎」把浮在每個分頁上的**底座**算了進去，
  於是它永遠 ≥3。**反向驗證當場證明它是死的**：把整個 Lab 的琥珀拿掉，
  數字從 10 掉到 3，斷言照樣綠。判準：反面斷言的**分母**要排除「無論如何都存在」
  的共用 chrome，否則它保證的是「chrome 還在」而不是「我做的事在」。
- ⚠️ **`.screen` 用 `opacity:0` 藏，不是 `display:none`。** 掃畫面時只問 display，
  五個分頁的元素會每次都被算進來 —— **徵兆是「每個分頁的數字一模一樣」**。
  要問 `el.closest('.screen').classList.contains('active')`。
- ⚠️ **漸層住在 `backgroundImage`，不在 `backgroundColor`。**
  2026-09-08 同一個漏洞出現**三次**（發光計數、動作盤點、可動層計數），
  每次都把全 app 最大的那顆漸層按鈕判成「沒有顏色」。
  順帶：解析時只抓 `rgba?()` 群組，直接抓所有數字會把
  `radial-gradient(circle at 30% 30%, …)` 的百分比當成色值。
- 🔴 **守門把規則套錯時，把判準改寫成「雙向等價」而不是放寬它。**
  同一天實例：「每一面都要有可動層」在 Timeline 上紅了 —— 而 Timeline 實測
  **0 個可點元素**，是純閱讀面。正解不是把 Timeline 加進白名單，而是改寫成
  **「有動作的面才要有」**（`clicks > 0 === amber > 0`）——
  這樣它會自己修正：哪天那一面長出按鈕而沒鋪，它照樣會紅。
...
- 🔴 **同一個指標，量測條件不同就不是同一個量 —— 不能進同一個百分位／基線池。**
  混池的徵兆**不是壞掉，是「看起來完全正常」**：短窗的離散度本來就大於長窗，
  併池會系統性灌大分母，讓所有人的分數往 50 收。比明顯的 bug 危險得多。
  2026-08 實例（PR #225，程式未併入，教訓留在這裡）：enrollment 的 **3.6 秒**掃描
  與日常的 **~10 秒**掃描曾被併進同一個 seed，並把「3.6s 比較短、噪音較大」
  寫成一個可以接受的限制 —— 那是寫錯的。
  ⚠️ **而且同一個錯會在不同層再出現一次**：同一輪後來發現 tier A（讀 landmark 位移）
  與 tier B（讀全幀 luma 差）也不可比，founder 實走三次**全部落在 tier B** ——
  tier 會隨裝置與網路（MediaPipe 載不載得到）改變，**不能假設一個人永遠同 tier**。
  判準：每次寫下「這兩個量不可比」，立刻問它**在哪幾層**成立 ——
  一次量測之內／跨掃描的池／跨裝置／跨版本。**守住一層不代表守住其他層。**
  ⚠️ 認不出量測條件的樣本**直接丟掉，不猜**。
- 🔴 **百分位的端點不是罕見事件，是必然事件 —— 分母要多帶一份權重。**
  用教科書的 `count/n`，破紀錄那天 `below/total = 1.0`，夾完就是滿分；
  n 筆歷史裡**約有 `2/n` 必然壓在兩端**。改用 Weibull 的 **`i/(n+1)`**，
  端點才留得出空間。v2 的舊百分位實作正是這個病。
  判準：任何「把值換算成 0-100 名次」的函式，先問**最大值那天會印出什麼**。
...
| 症狀 / 情境 | 規則 |
|------------|------|
| 用 `preview-shot.mjs` / 自寫的靜態伺服器截圖或跑 harness | 🔴 **先確認正式路由改寫過，否則會截到一個「JS 一行都沒跑」的死頁面。** 頁面裡是 `<script src="/preview/...">`（正式路由），伺服器認的是 repo 路徑 → 全部 404，但 HTML 是靜態的、**照樣渲染**，截出來的圖看起來很正常。⚠️ 比「截不到」危險：截不到會發現，截到死頁面不會。2026-09-16 實例：截 decision-alert.html 看新入口，副標空白差點被當成 bug 查，真相是 decision-outcome.js 404 → 頂層 `TENKI_OUTCOME.STORE_KEY` 拋錯。**同一個坑會讓斷言假綠**：同一天 preview-drift 的「沒有累積時說尚未累積」綠著，它量到的是 HTML 裡的預設字。→ 兩件事都要做：伺服器補 `/preview/`→`/apps/preview/` 改寫；**凡是由 JS 寫的格子，HTML 預設值不得等於 JS 會寫的字**（否則 JS 沒跑也會過）。 |
...
| 改了某段畫面的**呈現**（散文 → 欄位表、換分隔符、拆節點），而既有斷言是拿整段 `textContent` 比字面 | 🔴 **會紅的那條旁邊，常常站著一條默默失效的 —— 修紅的時候要把同一組都看一遍。** 2026-09-05 實例：收束頁把「同標的更新：1 次」改成標籤/值兩個 span（中間沒有冒號了），`/同標的更新：1\s*次/` 當場紅 —— 而它隔壁那條 `/同標的更新：0\s*次/` **變成永遠綠**：冒號不存在，就算產品真的印出「0 次」（把「不知道」講成「沒發生」的老傷）它也抓不到。**紅的那條會叫你，失效的那條不會** —— 所以只修紅的等於把守門默默拆掉一半。判準：斷言**正面**事實（值等於什麼）改呈現會紅，你會發現；斷言**否定**事實（不得出現某字串）改呈現會靜默失效，你不會發現 —— 否定型的字面斷言風險高得多。修法是問**結構**不問字面（依標籤找到那一列、讀它的值），呈現再變也不會誤判。 |
...
| 加了一條斷言，覺得它守住了某個取捨 | 🔴 **去破壞那個取捨，看它會不會紅 —— 不是破壞別的東西。** 2026-08-28 實例：我加「中間的時鐘仍是 1 行」，以為它守住「模板欄變寬會擠爆中間欄」；把上限從 72px 灌到 260px 之後**零條紅**，因為 `max-width` 只是上限、欄位是內容寬，名字沒變長欄位就不會變寬。斷言本身沒錯，但它**守的不是我以為的那件事** —— 這種誤解會讓下一個人放心地改壞它。**規則**：斷言的註解要寫「它擋得住什麼」，而那句話要有一次真的紅過當證據 |
| 斷言的對象是「既有就已經是這樣」的行為 | 🔴 **把「我沒改壞」寫成「我修好了」是最難看的一種假綠。** 實例：ready 狀態的中間欄印的是模板名，自訂名字太長就折行 —— 我一度把「時鐘不准折行」寫成新斷言，但實測**改動前後（72px / 100px）一樣折**。判準：新斷言在**改動前**跑一次；改動前就紅、改動後才綠，才是我修的；兩邊都紅就是既有問題（記進註解或 MEMORY，不要混進這一輪的功勞）|
| 測試裡「選一個東西」然後量它 | 🔴 **先把「真的選到了」寫成一條具名斷言。** 選不到時後面每一條都是**空過**（全綠而什麼都沒驗）。實例：自訂模板在 sheet 裡的 `data-id` 是 `CUSTOM_<id>`，我用裸 id → `pickTmpl` 回 false、底座維持預設模板、底下三條全綠。同理，掃一組東西之前要先斷言「**會出事的那一個**在清單裡」（六個模板裡只有 Canslim High RS 會被切，少了它整段就是裝飾品）|
| 同一個元件有兩種模式（倒數／守望、線上／離線…），而某個欄位只在其中一種下有意義 | 🔴 **不能在另一種模式下照樣印它的預設值。** 2026-09-03 實例：底座左欄無條件印模板的倒數時長，但結構守望根本不倒數 —— 它的界線是 30 分鐘殭屍上限。founder 實走放 6 分鐘，截到**同一列**左邊寫「3:00」、中間寫「06:16」。⚠️ 那個函式自己的註解就記著同一個坑的第一次（格式化算錯讓 3:30 印成 3:00），**但形狀不同**：第一次是算錯，第二次是「這個量在這個模式下不存在」。**判準**：換模式時逐一問「這一格的量在新模式下還存在嗎」；不存在就換成該模式**真正的界線**（並講明是「上限」不是「目標」），或不印。斷言要**兩邊一起鎖**（守望印上限 ∧ 倒數印時長），否則「乾脆全部都印上限」也會通過 |
| 畫面要講「這一次怎麼跑」，而程式裡有一個**設定**也在講同一件事 | 🔴 **用設定去代表事實，在兩者被刻意分開的那條路上一定會錯。** 2026-09-03 實例（上一條的續集，同一天）：我把底座左欄的界線改成問 `watchMode(tmplId)` —— 那是 Lab 的決策紀律開關＋是不是交易者模板，一個**設定**。但一筆正在跑的決策，它的界線是 `sess.watch`，**那一次跑法的事實**；而 `acceptHandoff()` 明文寫著「快訊交棒過來的決策一律 `sess.watch = true`，**不看那個開關**」。結果：手動起跑那條路修好了，快訊那條路照樣印「3:00」，同一列同時寫著「結構守望」與「3:00」。**判準**：正在跑就問那一次的 session 狀態，沒在跑才退回設定（設定只是「接下來會怎麼跑」的最佳預測）；尤其當程式裡有一條路**明文寫著「這裡不看那個開關」**時，那行註解就是在告訴你兩者會分岔。⚠️ **配套（漏掉的其實是這半邊）**：斷言必須走**會讓兩者分岔的那條路**。第十四輪的守門走手動路徑（開關永遠開著），設定與事實永遠一致 —— 它綠得理直氣壯，卻一次都沒測到會壞的組合。只走兩者一致的路徑等於沒有守門。⚠️ 順序陷阱同時存在：`setState('running')` 內就會重畫，而 `sess.watch` 是**之後**才設的 —— `acceptHandoff()` / `resumeActiveDecision()` 設完都要再重畫一次（兩處都反向驗證過，各自單獨拿掉都會紅） |
| 容器是圓的（`border-radius:50%`），而斷言拿它的 `getBoundingClientRect()` 去比 | 🔴 **那條斷言等於不存在。** 「在方框內」與「在圓外」可以同時成立 —— 尤其是掛在圓下半部的第二行文字，圓在那裡已經收窄。2026-08-28 實測：環心的過期文案超出**圓** 12.5~13.7px，而方框斷言在 7 個尺寸上**全綠**。⚠️ 最毒的是**反向驗證也會一起綠**（破壞與斷言共用同一個錯誤幾何），所以它可以活好幾輪沒人發現 —— 這條就是這樣活了兩輪。**規則**：圓形容器逐行取 `getClientRects()`，每個矩形的四個角問 `hypot(角 − 圓心) − R`。容差要從**行高推導**（12px 字放 14px 行盒 → 高估上限 2~3px → 取 4px），**不得從「剛好通過」反推**；有狀態超過容差就去修產品，不要放寬容差。驗收時要多做一步：**把破壞留著、只把斷言改回方框，它必須變綠** —— 那才證明換幾何是有意義的，不是換個寫法 |
| 兩個都吃 `vw` 的東西（容器與它裡面的字）各自設了 `clamp()` 上限 | 🔴 **兩個上限要在同一個寬度收手**，否則中間那一段就是「字還在長、容器不長」。實例：環 `min(54vw,208px)` 在 **385px** 封頂，但字級 `clamp(24px,7.2vw,30px)` 要到 417px 才封頂 —— 385~417 之間字持續變大而圓不變，角落被推出圓外 6.2px。算法：用容器封頂的那個寬度回推字級上限（7.2% × 385 ≈ 27.5px）|
...
| 一條守門守的是**跨全站的 bug 家族**，而你只掃了一個容器 | 🔴 **家族守門的掃描範圍要等於家族的居住範圍 —— 只掃一個房間，等於宣稱整棟樓都乾淨。** 2026-09-08 實例：「換模板不得改變畫面顏色」（紫時鐘那個 bug 的守門）只掃 `#today-screen` + `#fdcb`，而同一個根因（`var(--primary)` ＝ 當下哪個模板在跑）在 **Lab 有八個出口**：`.lab-item .ic` 讓九塊磁磚的圖示 Mancini→紫、Health Stress→綠、Exercise→橘。founder 用截圖發現，守門全程綠。擴到五個分頁之後**立刻紅在 Lab 的 16 個節點**，而且紅的 key **全部是 `lab/*`** —— 那正是「舊範圍抓不到」的證明。⚠️ 擴範圍時有兩個會讓斷言默默死掉的陷阱：①`.screen` 是用 `opacity:0` 藏的**不是** `display:none`，不切分頁就會每次都掃到全部五頁的元素（症狀：每個分頁數字一模一樣）；②浮在所有分頁上的固定元素（`#fdcb`）會進每一個分頁的分母，讓「這頁有沒有琥珀」那種比例型斷言永遠成立 |
| 一個顏色寫在 `radial-gradient()` / `linear-gradient()` 裡 | 🔴 **它住在 `backgroundImage`，不在 `backgroundColor` —— 只比對後者的斷言看不見它。** 2026-09-08 實例：`.snap-hint .sh-dot` 把 `var(--primary)` 藏在 `radial-gradient(circle at 38% 35%, #d8f7ff, var(--primary) 72%)` 裡，逃過了整輪的顏色掃描。⚠️ 解析時**不要**把字串裡所有數字都當顏色抓 —— `circle at 38% 35%` 的百分比會被算成色值；只匹配 `rgba?()` 那幾組。這個洞在同一輪出現了**三次**（不同的斷言各踩一次），所以它不是筆誤是**預設會犯的錯** |
| 一整頁的元素「什麼都可以按」，而你想用一個顏色標出「可以動手」 | 🔴 **在全部都可點的頁面上，把每一塊都標成可動層等於沒有標 —— 標的要是「會改變狀態」，不是「按得下去」。** 2026-09-08 實例：Lab 九塊磁磚每一塊都鑲了琥珀邊，結果整頁只剩一個顏色，founder 的反應是「顏色好像有點少」的另一種版本。founder 拍板改成只給**就地改變一件事**的（開關、匯出），**導航進子畫面不算**。⚠️ 同一頁的磁磚常常不是同一種東西：有真資料的 / 會改狀態的 / 只是導航的 / **根本還不存在的** —— 先分類再上色 |
| 一塊 UI 點下去只彈一個 toast（「即將開放」），卻和能用的功能並排同權 | 🔴 **那是畫面在承諾一件不存在的事 —— 要嘛拿掉，要嘛降級並明寫。** 2026-09-08 實例：Lab 九塊磁磚裡有**三塊是空 stub**（穿戴裝置 / Edge Score Bucket / 情境行事曆），其中「穿戴裝置」連 BETA 徽章都沒有，看起來完全可用。founder 拍板降級成獨立的「即將開放」區：**拿掉全部可動訊號**（不上可動層色、圖示不上色、邊框最弱、逐塊加徽章），仍然看得到（使用者要知道路線圖），但不再假裝可用。⚠️ 判準是**點下去會不會發生事**，不是它有沒有掛 BETA 標籤 |
| 大範圍搬動 markup（把 N 個區塊重新分組） | 🔴 **不要用「從名字往回抓幾行」的方式切塊 —— 要真的解析出每一塊的邊界，再逐塊確認你抓到的是誰。** 2026-09-08 實例：我把 Lab 八塊磁磚重新分組，helper 從名字往前抓固定行數，結果**搬錯了三塊** —— Baseline（有真資料）和 CSV 匯出（真動作）被丟進「即將開放」，而兩塊空 stub 留在主格線。改動本身編譯得過、harness 全綠、**只有截圖看得出來**。修法：先解析成 `{名字 → 完整區塊}`，印出名字清單肉眼核對，再照新順序重組 |

| 一個全域 CSS 變數，在 runtime 被 `setProperty` 覆寫成「當下的某個東西」 | 🔴 **那不是變數，是一條漏水管 —— 每一個 `var()` 消費者都被牽連，而它們大多不知道自己被牽連。** 2026-09-08 實例：`--primary`（品牌 cyan）在決策起跑時被覆寫成**當下那個模板的顏色**，於是全頁 60 幾處 `var(--primary)` 一起改色：時鐘變紫（**1.94:1**，連大字門檻都不到）、Lab 九顆圖示變紫、滑動膠囊的點變紫。三個 bug、三輪修，其實是同一條管子。修法是**把兩個職責拆成兩個 token**（`--primary` ＝ ACTIVE 永不變 / `--tmpl` ＝ 這一次的身分，可以變），並讓新的那個**預設等於舊的** ⇒ 沒被設定時是恆等變換（跟星塵 `setTone` 同一個結構性質）。⚠️ 判準：一個 token 如果會被 runtime 改值，它的名字必須講「這是誰的」而不是「這是什麼角色」 |
| 要做一套「身分色 / 分類色」（每個模板、每個專案、每個使用者一個顏色） | 🔴 **先把現有那組全部量一次，再決定要不要保留它** —— 這種調色盤最常見的壞法是「它根本沒在識別」。2026-09-08 實例：六個模板身分色裡**三個是同一個 `#00B4D8`**（而那還同時是 cyan ACTIVE 與 Clear 帶位），另外三個各偷一個語義主人（success 綠 / error 紅附近 / 一個 1.94:1 的紫）。**六個裡沒有一個是乾淨的。** 而且答案不是「換六個色相」（那是玩具調色盤）—— 是找出資料裡**本來就存在的分組**（這裡是守望 vs 倒數兩種跑法），做成 2 族 × 3 階：族間 ΔE 50、族內 8~10。**顏色講「哪一類」，名字講「哪一個」** |
| 想讓某個顏色成為新的語義色 | 🔴 **先過色盲，而且要接受「這個色相沒有可行解」是一個真的答案。** 2026-09-08 實例：紫要當 Premium，第一版 `#B081E1` 在綠色盲下與 Clear cyan 只差 **ΔE 3.2**，第二版高彩度版仍然 6.0；掃過整個紫色弧 295–325° × L\*38–88 × C\*25–110，**一個都活不過 ΔE 20**（最好 16.5，卡在紅色盲下的 Neutral）。紫失去紅分量就變藍。→ 正解不是降低標準，是**讓它當表面**：只給 wash/邊/填色的階、不給 `-400`，宣稱由**字**承擔。並加一條斷言「`--premium-400` 不得存在」把這個決定鎖住 —— 否則下一個人只會再試一次同一個死路 |
| 要斷言一個「疊出來的」顏色（多層半透明漸層、`color-mix`、mix-blend） | 🔴 **只有畫素答得出來 —— `getComputedStyle` 回的是宣告，不是結果。** 2026-09-08 實例：環境層是三層 `color-mix(... var(--tmpl) …%)` 疊在星雲上再疊星點，「它到底多亮 / 會不會被讀成某個帶位色」沒有任何一條 CSS 宣告回答得了。修法：`scripts/lib/png.mjs`（60 行的 PNG 解碼器）+ `page.screenshot()` → 直接取畫素。⚠️ 兩個配套：①取樣要**取一小塊的中位數**，單點會被星點那種亮的單一畫素帶偏；②要量「文字底下的背景」就**把文字 `visibility:hidden` 再拍**，在有字的位置取樣會取到字本身，那是死量測 |
| 寫「先設定某個狀態，再量那個狀態」的斷言 | 🔴 **先確認你真的在那個狀態裡 —— 被 `setTimeout` 延後的狀態切換會靜靜地把你打回去。** 2026-09-08 實例：`selectTmpl()` 會排一個 **260ms 後的 `setState('ready')`**，所以每一個「選模板 → `setState('running')` → 等 700ms → 量」的斷言，量到的其實是 **ready**。畫面在那 260ms 內確實是 running，截圖也看得到 running 的時鐘 —— 但量測發生在之後。**所有跟 running 有關的顏色斷言因此從來沒有走進去過。** 同一個 helper 還有第二個坑：`selectTmpl` 在 running 時 hard-return，所以連續量兩個模板時第二個**根本沒換過去**。修法是把兩件事都吃進 helper（先回 idle、選完等 timer 過去），而不是每個呼叫端各自小心 |

| 版面在「加入主畫面的 PWA」裡跟瀏覽器分頁裡長得不一樣（狀態列壓到內容、底部被 home indicator 吃掉） | 🔴 **安全區與內距是兩件事，要相加，不是二選一。** 2026-09-08 實例：`--top-safe:14px` 是「內容與螢幕邊緣的呼吸」，但 standalone PWA 的 iOS 狀態列**會蓋在頁面上** —— founder 實走截圖裡 `14:31` 正壓在「DEEP SCAN」膠囊上。改成 `calc(env(safe-area-inset-top, 0px) + 14px)`：Safari 分頁裡 inset 是 0，維持原樣；PWA 裡才長出來。⚠️ 兩個配套：①沒有 `viewport-fit=cover` 的話 `env()` **恆為 0**，改了也不會動；②**先看隔壁那一頁**：`decision-alert.html` 早就是這個寫法，這又是一次「同一件事兩頁各做各的」 |

| 把一條既有規則**收窄**（「可點就上色」→「只給會改變狀態的」），或新增一條紅線 | 🔴 **收窄規則的當下就要問：它現在守的範圍，還等於它宣稱的範圍嗎？** 2026-09-09 實例：founder 前一天把可動層琥珀收窄成「只給會改變狀態的，導航不算」，那一輪只掃了 Lab —— 而 `/decision-alert/` 的收束頁**剛好相反**（兩顆導航穿琥珀、三顆會寫進紀錄的自評晶片穿中性）。同一天另外兩個問題也是同一個形狀：「內部 id 不得上畫面」那條斷言**只掃模板選單**、沒掃收束頁；而這一頁既有的琥珀守門問的是「只出現在**可點**元素上」，兩顆導航當然可點，所以**全程綠著**。**三個問題都不是缺規則，是規則的掃描範圍比它的宣稱小。** 判準：規則一改，就把它的**所有消費面**列出來逐一走過，並讓斷言的範圍跟宣稱一致 |
| 想寫一條斷言去守「這個東西會不會改變狀態 / 有沒有副作用」這類語義 | 🔴 **機器判斷不出來，所以不要假裝偵測 —— 改成要求把決定寫成名單。** 2026-09-09 實例：「琥珀只給會改變狀態的」沒辦法自動判定，於是斷言改成「穿琥珀的節點集合必須**恰好等於**列舉的那些」。新增一個用法就會紅，逼下一個人回來說明它改變了什麼。這比一條偵測式的近似（「只出現在可點元素上」）誠實得多 —— 後者在該頁全程綠著卻什麼都沒守到。⚠️ 名單型斷言要**雙向**：名單外的要紅，名單內的也要真的還在（否則變成空頭支票） |
| 用 `el.hidden = true` 藏一個元素 | 🔴 **`[hidden]` 的 `display:none` 來自 UA 樣式表，同一個元素上任何 `display` 宣告都會蓋掉它。** 2026-09-09 實例：`.result-strip{display:flex}` 讓 `strip.hidden = true` 只變成一個 0 高的 flex 容器 —— 元素還在版面裡、`margin-top:10px` 也還在。實測 `getBoundingClientRect()` 回 `w:352 h:0`（真的 display:none 會四個值全 0，那也正是分辨的方法）。修法：補一條 `.foo[hidden]{display:none}` |
| 想沿用站內某個「已經驗過」的**浮層位置**（bottom 座標、z-index 那一組） | 🔴 **連同抄「那個位置為什麼安全」，不要只抄座標。** 2026-09-10 實例：`.watch-judge` 的註解原文就寫著「位置與收合方式抄 `.tp-picker`（已驗證過的那組）」—— 抄了 bottom 座標、抄了 `visibility` 收合，**沒抄那塊 `--n-900` 的不透明底板**。而 `.tp-picker` 能站在那個座標上靠的正是底板：那條帶在多數可視高度下壓在「示意說明／輪播圓點／vitals 卡」上。於是判定列的兩顆按鈕之間，底下那行字從縫隙透出來，founder 實走截圖看到的就是字壓在字上。判準：抄一個位置之前先問**它憑什麼可以站在那裡** —— 是空的？有底板？會把內容推開？把那個理由一起抄過來 |
| 要斷言「這個浮層有沒有把底下的東西蓋住 / 透出來」 | 🔴 **不要用 `elementFromPoint` —— hit test 不看背景透不透明，一個沒有背景的 `div` 照樣吃掉點擊。** 2026-09-10 實測：把 `.watch-judge` 的背景改成 `transparent`，在兩顆按鈕之間的縫隙（390×740 下是 195,569）問 `document.elementFromPoint`，回的仍然是 `watchJudge` —— 那條斷言改壞了照樣綠，是死斷言。要問的是真正讓「覆蓋」成立的性質：**`backgroundColor` 的 alpha 為 1**。守門寫成掃性質（`.phone` 裡所有 absolute/fixed、z-index ≥ 45、與內容矩形相交的元素）而不是寫死名單，下一條浮層加進來會自動被守到 |
| 掃版面高度去驗遮擋，尺寸清單用的是 iPhone 的規格高度（844 / 852 / 932） | 🔴 **那是裝置高度，不是視窗高度 —— Safari 與 App 內建瀏覽器扣掉網址列與工具列之後落在 700~780，而那正是最擠的一段。** 2026-09-10 實例：founder 截圖裡浮層壓在說明行上，我用 844/852/874/896/932 掃**全部回報乾淨**，一度以為重現不出來；補掃 640~800 才發現 390 寬 × 15 個高度裡**有 11 個會撞**，不撞的只有 820~874 那一小段 —— 也就是說「乾淨」是設計高度 844 的巧合，不是保證。⚠️ 附帶一條：活性證明要交給**會撞的那些高度**；拿一個本來就掃不到東西的高度去證明「斷言活著」，證出來的是反面 |
| 為了「誠實」而加一個旗標欄位（`staleAtDecision`、`isEstimated`、`partial`…） | 🔴 **寫進去不等於有人讀 —— 誠實規則寫在 doc comment 裡不會自己生效。** 2026-09-10 實例：決策紀錄照契約寫了 `staleAtDecision`，而消費端 `bandOfRecord()` 只問 `band` 存不存在。結果 founder 的 Baseline 是「49 小時前 校準」，一筆用它跑完的決策被歸給 **Clear**（實測 `attributed:1 excluded:0`）—— 正是那支 policy 的 doc comment 自己警告的 "guessing one would fabricate the very insight this exists to give"。⚠️ 這是「宣告了沒人用」的第二次（第一次是 `AttachedReadinessReading` 這個介面本身）。判準：新增一個旗標的**同一顆 commit** 就要有一個消費者 + 一條會紅的斷言，否則它只是一份寫給未來的備忘錄。⚠️ 閾值不要在消費端另訂 —— 旗標在**存檔時**用既有常數算好，讀端只讀它，不然同一個「過期」會有兩個定義 |
| 一張統計圖排除了一部分資料，而排除有**不只一種理由** | 🔴 **理由要分開講。合成一句就是把「不知道」講成「沒發生」，而且會讓配套的那句建議變成假的。** 2026-09-10 實例：校準台原本一律印「N 筆決策當下沒有讀數」，底下配「**先掃一次再進決策**」。但被排除的其實有兩種人：真的沒掃過的，和**掃過但太久以前**的 —— 對後者那句建議是假的（他掃了，只是過期）。改成兩個數各自印，並讓邀請語跟著分岔（有過期紀錄 → 「進決策前先掃一次」）。判準：每多一種排除理由，就多一個計數欄位；並加一條「三個桶加起來等於總數」的斷言，確保沒有人被漏掉或重複算 |
| 一句**帶分隔號的短語**（「判定不成立 · 未進場」）放進一個窄容器，而它斷行了 | 🔴 **先分清楚「溢出」與「斷錯地方」—— 那是兩個 bug，修法相反。** 2026-09-11 實例：founder 說收束頁環心「有點裁到了」。逐行四個角對圓心量的結果是**沒有**溢出（176px 弧下最差 −17.3px，全在圓內）；壞的是斷點 —— 整串要 135px、容器內容框 132px，**差 3px**，於是斷成「判定不成立 · 未進」／「場」，一個字掉在第二行，看起來就是被裁。修法是 `word-break: keep-all`（CJK 之間不再是斷行機會，只剩分隔號兩側的空白可斷），**不是**把字縮小、更不是改文案 —— 那三句是 `OUTCOME_VIEW` 單一來源，同時給底座與 Session 徽章用，為了一頁版面去動它會牽連另外兩頁。⚠️ 守門不要寫成「行數 ≤ 2」：切在詞中間同樣是 2 行，那是死斷言。要問**最窄的一行不得比最短的一段還窄**，而「段」用 Range 去量真實寬度、不用字數估 —— 這樣它跟著文案走，沒有魔術數字 |
| 走訪 CSSOM（`document.styleSheets` → `cssRules`）去掃規則 | 🔴 **不要用 `if (r.cssRules)` 當「這是容器」的判準** —— 支援 CSS nesting 的瀏覽器上，一般的 `CSSStyleRule` **也有** `cssRules`（一個空的 `CSSRuleList`，而空 list 是 truthy）。2026-09-21 實測：903 條頂層規則只走到 **93 個葉節點**，走到的那 93 個還是 keyframe / font-face 這類真的沒有 `cssRules` 的 —— 也就是說那條掃描從寫下的那天起就幾乎沒在掃東西。正確寫法是 `if (r.cssRules && r.cssRules.length) walk(...)` 並**不要 `continue`**（規則自己的宣告仍要看）。⚠️ 這條的教訓比 bug 本身重要：它旁邊有一條活性檢查（另外數 `sheet.cssRules.length`）而且**一直是綠的** —— **活性檢查量的不是同一個東西，就不是活性檢查**；要量就量「這次掃描實際命中了幾條」 |
| 盤點「某個顏色用在哪裡」 | 🔴 **掃值，不要掃 token 名字。** 2026-09-21 實例：我發表了一份 `--good` 綠的盤點，執行時發現**漏了六處** —— 因為盤點只 grep `var(--good)` 加兩種字面，於是每一條走別的路的綠都躲過去了：**第二個同值 token**（`--fdcb-complete:#34C759`，底座收束態那圈綠就是它）、`linear-gradient` / keyframe 的**顏色停點**、**inline `style="color:…"`**、以及 **JS 傳進 canvas 的字串**。判準：一個顏色的「用量」要沿著值去找（含 token 間接），而且**要講清楚掃描到不到得了 JS 畫的東西** —— 到不了就把那部分寫成刻意保留的名單，不要讓斷言的宣稱比掃描大 |
| 一條守門抓到東西，而抓到的看起來不像違規 | 🔴 **先問「是規則錯了，還是範圍寫得比它想守的事大」，不要急著改產品去迎合斷言。** 2026-09-21 實例：琥珀近似色那條修好之後抓到 Hero 巨環的 conic 漸層 —— 但琥珀守的是「可以動手」這個語義不被借用，那是**儀器世界**的事，而巨環是世界 A 的大面積漸層，掃過金色不等於在說「這裡可以按」。正解是**具名例外 + 寫下理由**（判準同星塵那條「守的是整顆球的主色，不是每一顆粒子」），不是把 founder 每天在看的 Hero 重新上色。⚠️ 例外要雙向：名單外的要紅，名單裡的也要有一條「例外都還在」擋空頭支票 |
| 幫一條規則開例外 | 🔴 **例外要精確到「哪一個宣告」，不是「哪一條規則」。** 2026-09-21 實例：綠的守門放行整條 `:root`（理由是 `--good` 的宣告必須留給 onboarding 用），結果反向驗證「把綠塞回 `--fdcb-complete`」**沒有紅** —— 新的綠 token 藏在同一條 `:root` 裡，被自己的例外蓋掉。改成鎖住**綠 token 的名單本身**（就這兩個，多一個就紅），同一個破壞立刻紅 |
| 要立一條「某個顏色不准用在哪裡」的規則 | 🔴 **判準要問「資訊由誰承載」，不要用「它出現在哪一頁」代替。** 2026-09-24 實例：綠的規則原本寫成「只准出現在 `.baseline-flow`」—— 那只是當時綠剛好都還留在那裡，位置不是理由，於是 founder 想讓壓力條用綠時，規則除了開例外沒有別的話可說。改成一句可驗的問句：**「把顏色全部拿掉，這個元件還說得出同一件事嗎？」** 說不出（一顆沒有字的點）→ 不准；說得出（5 格亮 3 格、旁邊有字）→ 可以。⚠️ 附帶的好處是它解釋得了為什麼同一組色盲下會塌的值，在 A 處不可用、在 B 處沒問題 —— 壓力條那三階紅色盲下 ΔE 只有 5.5，但訊息是**位置**，所以不影響。⚠️ 名單型守門要跟著改：名單的每一列都要寫出它為什麼過得了那個問句，而不是只列出選擇器 |
| 看到 `linear-gradient` 就以為那是一條漸層 | 🔴 **同一個漸層套在 N 個等分的子元素上，是同一個圖案重複 N 次，不是一條帶。** 2026-09-24 實例：壓力條五格各自套 `linear-gradient(90deg,#34C759,#8BD944)`，五格長得一模一樣 —— founder 說「我要有漸層色」時，原本那個其實從來沒有漸層過。要讓整條讀起來是一條帶，得**逐格遞進**（`nth-child` 各給一階）；中間階用 `color-mix()` 從端點推，不新增 hex |
| 一個欄位在某些情況下沒有值，而你打算印「—」／空字串把它佔住 | 🔴 **沒有值的欄位不要硬留，但一定要就地說出為什麼沒有。** 2026-09-25 實例：模板表的 `MATCH` 欄永遠空著 —— founder 的真快訊是「ES1! 下穿 7,740.00」，`strategyHint` 是空的。我提議印破折號，他當場否決：「印『—』這樣看起來會像壞掉」。他是對的 —— 破折號只是把空白換一個寫法，**欄頭仍然在承諾一個它給不出來的值**。改成第三欄**整欄不渲染**（表頭兩格、每列也不多一個空 div），並補一句「這則快訊沒有指名結構 —— 三個流程都可以選」。⚠️ 這跟 §6 那條「不支援就整列隱藏」不是矛盾而是同一條的兩面：**沉默會誤導，半句話也會** —— 拿掉的是那一欄，不是那個解釋。⚠️ 守門要有**對照組**：只驗「沒有比對到時沒有第三欄」的話，一個「永遠不畫第三欄」的 bug 可以讓它全綠 |
| 某個欄位的值來自「快訊／外部 payload 的某一欄」，而畫面上它總是空的 | 🔴 **先問那一欄在真實 payload 裡有沒有人填，不要只看 demo。** 2026-09-25 實例：結構比對只讀 `strategyHint`，而 demo 按鈕寫死 `'Mancini'`，所以**比對失敗那一格在 demo 路徑上根本走不到**，看起來一直是好的。真實 TradingView 快訊把結構寫在**訊息欄或條件欄**。修法兩件事：比對改吃 `strategyHint + condition + note`；harness 改走 **live 路徑**（塞 `tenki.alert.channel` + `page.route` 攔 `/api/alerts`），那是唯一能餵出「demo 造不出來的形狀」的辦法 |
| 想在關鍵字表裡加「下穿 / 上穿 / 跌破」這類**方向詞**，好讓比對率高一點 | 🔴 **只收指名結構的詞。方向詞只說了價格穿過一個數字，沒說這是不是一個 failed breakdown —— 而那正是使用者要守望的事。** 把方向詞映射到某個流程，等於讓 App 替使用者猜一件**還沒發生**的事，那是這條產品線最不能做的動作。比對不到是**正常情形**，畫面照實說就好 |
| 寫了一條守門，跑過、綠了、收工 | 🔴 **先確認那支 harness 真的在 `scripts/verify.sh` 與 `.github/workflows/ci.yml` 的清單裡 —— 不在的話，那條斷言從第一天就不會跑。** 2026-09-25 實例：gold 的真值表與接線守衛第一版寫進 `preview-scan-stardust.mjs`，手跑全綠 —— 而那支**兩邊都沒有**（檔頭寫明它倚賴「容器連不到 cdnjs」這個前提，CI 連得到，前提反過來）。搬到 `preview-scan-blink.mjs`（同樣只需把模組載進一個頁面，而且兩邊都跑得到）。判準跟這一節其他幾條同源：**一條不會被執行的斷言，跟一條永遠不會紅的斷言是同一種東西** |
