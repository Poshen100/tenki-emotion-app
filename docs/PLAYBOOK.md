...
| 一個「使用者做了某動作就會回應」的偵測器，實機上像壞的（使用者說他做了，程式說沒有） | 🔴 **先算取樣率 vs 事件時長，再改門檻。** 2026-08-29 實例：Soul Lock 的眨眼確認連兩次報「未偵測到眨眼」，founder 明說他有眨。FaceMesh 每 `FACE_INTERVAL_MS`(180ms) 才推論一次，而一次眨眼閉合只有 **100–150ms** —— 眨眼常常整個落在兩次取樣之間；就算落到一幀，那幀多半是**半閉**（0.3–0.5），而判定要求 `< 0.25` 且前一幀 `> 0.55`，兩邊都不滿足。**門檻怎麼調都救不了取樣率造成的漏抓**，因為漏的樣本根本不存在。修法是改看**形狀**（掉到自己基線的比例以下 → 幾幀內回升），不是改絕對值 —— 順帶也讓「眼睛比較細的人」自動適用，不必為體質調參。⚠️ 判斷「這是不是取樣率問題」的方法很便宜：把結果頁上的幀數 ÷ 秒數算一次（這次是 `56 幀 / 10.0 秒` = 178ms），跟事件時長比 |
| 為了讓偵測「抓得到」而放寬門檻，而那個偵測的結果會被**寫成文案** | 🔴 **放寬靈敏度可以，放寬誠實度不行 —— 每放寬一格就要補一道防偽。** 2026-08-29 實例：眨眼改成谷底偵測後，「瞇眼」「低頭」「緩慢閉合」全都長得像谷。因為畫面會印「眨眼確認」（在宣稱一個事實，跟假讀數同一條紅線），所以補三道：①谷底必須在 N 幀內回升，真眨眼會立刻睜開；②沒回升的谷標記作廢但**不清狀態**，清掉的話下一幀會重新入谷、等真的睜開時補報一次假眨眼；③訊號源中斷（臉不見）就把谷斷開，跨空窗的「回升」不算。驗收要**兩排都有**：該抓到的要抓到、**不得誤判的那排更重要** |
| 某個只出現 1 秒的效果，founder 反覆問「它到底有沒有發生」 | **把它變成事後看得到的事實，不要叫人再走一次去盯著看。** 2026-08-29 實例：Soul Lock Beat 3 只亮 1 秒，走兩次都判斷不出踩到的是眨眼還是靜默保底 —— **那一拍等於無法驗證**，連 bug 都無從報起。把「這次是哪條路」寫進結果頁的規格行之後，下一次實走立刻定位出真正的成因。⚠️ 只在**真的有那個量測**的路徑上顯示（tier B 沒有眼瞼 landmark 就一個字都不提），否則就是宣稱一個不存在的量測 |
| 浮層／提示／膠囊要放在一張資料卡上 | 🔴 **卡片上沒有「空白區」—— 先量再放，而且不要只避開你上次撞到的那個東西。** 2026-08-29 是**第二次**：第一版壓在波形圖上（founder：「遮擋圖表」），我改放「標題列 ↔ 圖表」的中點，而那個帶裡站著數字本身（68 BPM / 49 ms）。資料卡由上到下是密的：標題列、單位標籤、大數字、波形。正解是移出卡片（錨到輪播圓點下方的真空區）並夾住下緣避開 dock。**驗收要用幾何斷言**（`scripts/preview-today-layout.mjs`）：浮層矩形不得與卡片／註記／圓點／dock 相交，也不得跑出視窗 —— 眼睛看「好像沒擋到」擋不住第三次 |
| 中文文案要放進一個**固定寬度**的容器（環心、膠囊、chip） | **算 max-content，別憑感覺。** 2026-08-29 實例：環心黑圓是 `.tl-edge` 的 62%，375px 手機上只有 ~112px，而文案含兩個**全形分隔符**（`　·　`，每個約 1em），max-content 到 152px → 斷在詞中間（「信心中 · 提／升精度 ›」）。修法：收短文案 + 收字級/內距 + `white-space:nowrap` —— nowrap 的作用是**當守門員**，讓下一個把文案改長的人看到明顯溢出，而不是安靜地斷詞。⚠️ 沙箱載不到 Inter（Google Fonts 被擋），量出來的寬度跟真機有落差 → 斷言要留安全邊際（現行 6px） |
| `@media (max-height:…)` 這類壓縮層在某些機型不生效 | **媒體查詢讀的是視窗，版面盒可能不是視窗。** 2026-08-29 實例：`.phone` 只在 `max-width:420px` 以下全螢幕，但 iPhone Plus／Pro Max 是 **430–440pt** → app 被 letterbox 在 390×844 的框內，而壓縮層讀的是**視窗高 932**、不是框高 844，於是壓縮不生效、內容撐出框，dock 蓋掉輪播圓點與示意註記。凡是「框住 app」的容器都要確認**斷點涵蓋所有真機寬度**，否則兩套判斷會各說各話 |
...
| 想截 RN 畫面給 founder ／ **founder 問「這頁要不要實機或圖書館電腦才能驗？」** | `cd apps/mobile && npx expo export --platform web`（offline 模式）→ 本地 serve dist（SPA fallback 到 index.html，否則子路由 404）→ Playwright 截圖。dev server（`expo start`）的 LogBox 在 web 會拉 RN internals，**用生產 export 別用 dev**。**版面／文案／狀態邏輯到此為止就驗得完，不必實機**；只有原生行為（相機、Skia、觸覺、真的權限視窗）才需要 Expo Go＋電腦。⚠️ `fullPage: true` 會拍出假白邊（`Dimensions.get('window')` 在載入時算的固定高度撐不滿），**用真機視窗尺寸截**。要看某個狀態長什麼樣（已連接／未授權／錯誤）就開一支臨時 route 灌 store，截完刪掉、不進 commit。（`/face-baseline/` 公開網址與 app.json baseUrl 已於 2026-07-03 退場 — dist 不再入庫，僅本地截圖用） |
