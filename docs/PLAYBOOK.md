...
| 本機 harness 全綠，同一份 code 在 CI 紅 | 🔴 **先問「這兩個環境對外連線不一樣嗎」。** 開發容器連不到 cdnjs / jsdelivr（`curl` 回 000），CI runner 連得到 —— 而 `/v3/` 在 splash 那段 inline script **之前**有四支 blocking 的外部 script。容器裡它們立刻失敗、頁面照走（**所有斷言都是在這個前提下寫、也是在這個前提下反向驗證的**）；CI 上每開一頁都真的下載，一支卡住後面的 inline script 就不執行，頁面永遠停在 splash。**規則**：harness 一律 `page.route(/(cdnjs\.cloudflare\.com|cdn\.jsdelivr\.net)/, r => r.abort())`，讓兩邊條件一致。⚠️ 這種修法**在容器裡驗不出效果**（那裡本來就連不到）—— 改成量「路由有沒有真的攔到」，不要宣稱驗過一個驗不到的東西 |
| `page.waitForFunction(fn, { timeout: N })` | 🔴 **timeout 是第三個參數**（`fn, arg, options`）。寫成第二個等於被當成傳給函式的 arg，逾時**默默**退回預設 30 秒 —— 失敗訊息會報一個你沒設過的數字，看起來像別的問題。要寫 `fn, null, { timeout: N }` |
| 想把一個浮動元素（提示、膠囊、徽章）疊在一張已經有內容的卡片上 | 🔴 **先問「這個容器裡有沒有它能站的地方」，不要挑座標。** 沒有空白帶的容器，任何座標都只是換一個受害者 —— #232 的滑動提示連續被修兩次（蓋住波形 → 改放「卡頭↔圖表中點」→ 蓋住 HRV 數字），因為 `.vcard` 由上到下是 `.vhead`/`.vlabel`/`.metric-num`/`canvas` 四層連續內容。**規則**：量出真的空白的那條帶（保留區、間距），量不到就**不要顯示**，不要退回比例猜測值 —— 猜出來的位置正是遮擋的來源。斷言要問「有沒有壓到容器裡的**任何一層**」，不是「有沒有壓到 founder 這次指名的那個元素」（只盯一個，把它往旁邊移 20px 就變綠，而它只是改壓隔壁）|
| 用 media query 的 `max-height` / `max-width` 控制版面尺寸 | 🔴 **條件要跟版面真正活在裡面的那個盒子對齊，不是跟視窗。** v6 的版面活在 `.phone{height:min(100vh,100dvh,844px)}` 裡，但壓縮那組寫的是 `@media (max-height:880px)` —— 視窗一比外框高（428/430 寬的 iPhone、**以及所有高於 880px 的桌機視窗**），版面就拿到一套為更大螢幕設計的尺寸，塞不進 844 的外框，輪播圓點整排掉到底座下面。**而桌機預覽永遠在那個破掉的分支上，所以「我自己看起來沒事」從來不算證據。** 修法是照抄那個盒子的規則（`@media (max-height:880px), (min-width:421px)`），**不是**換一個更大的魔術數字（`max-height:1000px` 只是剛好蓋過今天量到的機型，說不出為什麼是這個數）|
| 用 `elementFromPoint` 驗一個元素有沒有被蓋住 | 🔴 **它會直接跳過 `pointer-events:none` 的元素** —— 拿它問一個刻意設成 none 的元素（浮層、提示、裝飾），永遠回不到那個元素本身，斷言**恆紅**而且紅得看起來像版面壞掉。測視覺堆疊時暫時把它翻成 `auto`、問完翻回去；測「該不該吃點擊」時才照原樣問 |
