...
- **🔴 「元素在」不等於「有話說」—— 空字串照樣有高度**（2026-09-12，第三次同類）。
  ...
  **第六次（2026-09-17，驗收報告）改的是測資而不是斷言的右邊，但同一個病：
  harness 每次都先把 log 清空，所以它永遠只餵自己剛寫的那一筆。**
  於是新增欄位的 migration 判斷寫成 `c.exposure !== null`（舊紀錄那個欄位是
  `undefined`，`undefined !== null` 是 **true**）—— 報告對舊紀錄直接爆，
  而 harness 全綠。代價是**三次實機、三次都沒有任何數字回來**，而那三個數字
  正是當下唯一缺的東西。
  **問法再補一條：這組測資裡有沒有一筆是「不是我這個版本寫出來的」？**
  凡是讀持久化資料（localStorage / 檔案 / DB）的程式，測試一定要餵一筆**舊格式**的
  紀錄；`!== null` 不是「欄位存在」，要判就兩個都判。
  **第七次（2026-09-17，報告長度上限）改的是 fixture 的「代表性」：
  `report.split('\n').length < 24` 這條自從加了曝光段就一直是空的。**
  它跑的 fixture 的 `exposure` 是 `null`，整段塌成一行「還沒有任何量到曝光的擷取」，
  所以斷言從來沒看過一份真的報告。實測完整報告 **26 行** —— 它早就違反了
  自己要守的規則，而且是綠的。
  **問法再補一條：上限／門檻類的斷言，我餵的是最長／最壞形嗎？**
  凡是守「不得超過 N」的斷言，fixture 必須讓每一段都長出最長形，
  否則守的是一個不存在的短版本。
  **第八次（2026-09-17）不是斷言的問題，是 fixture 只有一個點：**
  拿「真 1.4 秒的漂移報成 3.58 秒」這**一個**量測，就推論「4 秒是安全地板」。
  掃過去才看到 0.8 秒報成 4.07 秒、1.2 秒報成 6.21 秒，全部過門。
  **問法再補一條：我驗這條門檻時，餵了幾個點？** 一個點不是一條曲線 ——
  門檻類的驗證要掃一整排參數，而且要把**兩側**都掃到。
  **第九次（2026-09-21）不是斷言錯，是註解吹過頭：** analyze 裡
  「只有節律才嘗試分段」那個合言，註解寫成「ordering here is the safeguard」，
  但**拆掉它不會弄紅任何測試** —— 真正擋住的是下游 per-segment 的灌流檢查。
  **問法再補一條：我說某條檢查是承重的，我把它拆掉試過嗎？**
  拆掉不紅就是縱深防禦，註解要照實寫；不要讓下一個人以為刪了另一道也沒關係。
