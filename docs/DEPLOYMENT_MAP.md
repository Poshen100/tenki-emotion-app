...
| `https://tenki-emotion-app.vercel.app/preview/reliability.html` | `apps/preview/reliability.html` | **量測重複性自測** —— 儀器自測，不是讀數（全程不顯示帶位）。連掃 3 次（中間刻意放開 15 秒讓每次獨立），比較同場離散度與跨天離散度。⚠️ 跨天顯著大於同場，Edge Score 的 z 模型才有意義 | 🔧 Founder 量測工具 |
