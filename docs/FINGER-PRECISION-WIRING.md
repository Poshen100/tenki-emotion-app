...
> 🔴 **2026-09-11 更新：這份文件的 §5 存錯了東西。** 它定義手指補強層只存
> `tenki.precision.hrBaseline`（一個 **HR** 基線）—— 但 HR 是臉掃也拿得到的
> 那一項。手指貼著鏡頭＋閃光燈唯一真正換來的是拍點時序精度，也就是 **HRV**，
> 而那項沒有被建立基線。定位與新方案見 `docs/PHONE-PPG.md` §10 與
> `docs/SOUL-SCAN-NORTH-STAR.md` §1 的 2026-09-11 例外條款。
