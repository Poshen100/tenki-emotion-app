...
**這份工單在 `main` 上（所以你找得到它）。它描述的程式碼不在。**
那 40 個 commit 只存在於分支 `claude/tenki-core-growth-arch-7teiqj`。
在 `main` 上跑 `npx expo start`，你會看到一個沒有這輪任何東西的 app。

**不要用 GitHub API 去找那條分支。** 這個 repo 有 **71 條分支**，未認證的 GitHub API
每小時只有 **60 次**請求。「逐條分支拉 tree 找檔案」會在中途被 rate limit 擋掉；
若迴圈把錯誤 `catch {}` 吞掉，你會得到「找不到」的錯誤結論，然後開始編造。
**2026-08-19 已經發生過一次** —— 那次產出的計畫主張用「模擬訊號管線」補上缺少的相機，
正好是本工單與 `choreography.ts` 的 `isQualityInstrumented()` 明文禁止的事。

正確做法：**你已經 clone 了整個 repo，用磁碟上的它。**
...
```bash
git log --oneline -1     # 126effe docs: stop the next agent from inventing a plan
ls docs/prompts/antigravity-expo-go-kickoff.md
```

⚠️ 這份工單在 `main` 與分支上**各有一份**，只有分支那份旁邊有程式碼。
