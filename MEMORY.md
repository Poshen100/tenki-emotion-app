...
# 2026-08-13 Session Update #75 (撐爆環心那行字的那一段，是同一個數字講第二遍)

founder 實走截圖（08:30）：功能全對了 —— 底座收成一行、Snapshot 線圖沒被蓋、
基線從 1/30 走到 2/30。壞的只有環心那行：

> `13 秒前 掃描 · 基線 2/30 · 2/7 天`

它撐破 `.tl-edge-center`（186px）換到第二行，「天」被擠成孤字壓在信心膠囊上方。

## 一、🔴 根因不是版面，是那行字在重複自己

`baseline-store.js` 的 `appendSample()` **同 profile 同一天後寫覆蓋**，
所以對任一 profile，`sampleCount` **恆等於** `distinctDays`。
實測（每天掃 3 次、連續 12 天）12 天全部相等。

於是「· 2/7 天」沒有帶任何新資訊 —— 它只是把 `2/30` 講第二遍，還讓人以為
有兩個獨立門檻，**而它正是把那行撐爆的那一段**。拿掉它，最壞情況從 38 欄
掉到 22 欄。

⚠️ 拿掉的是**顯示**，不是閘門：`MIN_DAYS_FOR_SCORE = 7` 留在 domain 當縱深防禦
（EDGE-SCORE-DEFINITION §3.3 的理由：那條去重規則住在另一個模組，
**依賴遠處檔案的保證不算保證**）。

順手拿掉的還有兩段在別處講過的話：「重新掃一次」（已在 `COACH_STALE_V6`）、
「到 Scan 掃一次」（已在 `COACH_NO_READING_V6`），以及「掃描」
（帶位詞 + 「狀態讀數」標籤下已是贅字）。

## 二、⚠️ 我的守門員為什麼沒抓到：沙盒的字體不是裝置的字體

沙盒量出來 **172px / 可用 186px，沒換行**；founder 的 iPhone 換行了。
差別在容器沒有 PingFang TC，CJK 被代換成較窄的字。

所以**版面寬度的斷言不能量像素** —— 那是在量另一種字體的答案。
改量與字體無關的**字元預算**（CJK/全形 = 2 欄），上限 28 欄由
186px ÷ 12px = 15.5em = 31 欄推導後留約 10% 餘裕。

⚠️ 第一版的欄寬掃描只掃了一種時間措辭（三態都是 `已過期`），**測不到最壞情況**。
擴成 21 態（所有 `agoTextV6` 措辭 × 樣本數位數 × 有/沒有讀數）之後才找到真正的
最壞情況：**沒有讀數那一支，38 欄**。

新增一條斷言釘住 `sampleCount === distinctDays`（9 天 × 每天 3 掃）——
**顯示能省掉天數的前提就是這個恆等式**。哪天有人拿掉一天一筆的去重，
`基線 n/30` 就會開始誤導（n=30 但只跨 2 天），這條會先紅。

## 三、教訓（已提煉進 PLAYBOOK §3 壞斷言表第六型）

**沙盒的字體不是裝置的字體** —— 版面寬度在容器裡量像素會過，上機才壞。
量與字體無關的字元預算。

## 下次接手點

- 🔴 **跨天再現性數字仍然沒有**：`/preview/reliability.html` 需要 founder
  隔天再跑一次。session 內 SD 0.0186 只是分母的一半 —— 若跨天 SD 也是 0.02，
  代表 86% 的變異是儀器噪音而非人的狀態。
- 未定：高 stillness 對應 Above 還是 Below Baseline（brand.md §7）。
- `weightedPercentile` 寫好測好但**刻意未接線**。
- `apps/mobile` 的 `deriveDailyEdgeScore`（`Math.sin(day)` fallback、
  把 confidence 當分數）是另一刀。

---

# 2026-08-12 Session Update #74 (決策底座：閒置的東西不該用執行中的尺寸付租金)

founder：「決策計時器 目前好像是作一半的狀態，各功能都在而且也會遮擋，
我也不知道它右側的點點按的數字代表什麼意思」。

## 一、功能是完整的，缺的是「它沒決定自己是什麼」

四態機、真計時器、模板選擇、`marks` 一路存進 store 並被 Timeline 的點半徑與
Session 詳情讀取 —— 全都通。但底座掛在 `.phone` 上、在 `.screens` **外面**，
橫跨 5 個分頁，用 `--fdcb-h:58px` **付 5 個分頁的永久租金**，儘管絕大多數時間
閒置。Timeline 的 Event Log 被切掉就是帳單。

解法是讓**尺寸與範圍跟著狀態走**：閒置＝入口（40px，只在 Today）、
執行中＝現場 HUD（58px，跟著跨分頁）。`--fdcb-h` 住在 `:root`，class 下在
documentElement 上，所有分頁的底部保留量自動跟上 —— 一改全解。

## 二、🔴 最尖銳的一點：它手上有資訊卻不用

idle 分支早就寫著 `segEl.textContent = '尚無讀數'`，卻仍然在旁邊說
「START DECISION」。**App 同時在說「我不知道你的狀態」和「去做決策吧」**，
牴觸 SYSTEM.md 的命題。改成沒有讀數時說「先掃一次」並導向 Scan。

## 三、⚠️ 我這一輪自己判斷錯的地方

我斷定「閒置仍渲染三段灰進度軌」—— **錯的**。
`.fdcb.state-idle .fdcb-prog{display:none}` 在我動手前就存在，
閒置時真正還在畫的只有事件群（兩顆點 + 一個假的可按 `+`）。
是**反向驗證**抓到的：刪掉我新加的規則，那條斷言仍然綠。

教訓：**反向驗證要刪掉自己那一份，不是弄壞整個行為。** 還綠就代表這條守的不是
你以為的東西 —— 而我已經照著那個錯誤判斷寫了分析、commit message 與計畫。

另一個：700px 時 `elementFromPoint` 對圓點回 null，我判成「被底座蓋住」，
實際上那段高度**本來就設計成會捲**（CLAUDE.md：短視窗不縮環、改讓畫面捲動），
視窗外的點本來就回 null。**把「要捲動」當成「被遮擋」會往錯的方向修。**

## 四、下次接手點

- 🔴 **跨天再現性數字仍然沒有** —— 需要 founder 隔天再跑一次
  `/preview/reliability.html`，這個 z 模型才驗得完。
- 事件（marks）目前只在決策進行中記得起來；要不要能回頭補記、要不要帶標籤，
  尚未討論。
- takeover 死碼還沒清。

# 2026-08-12 Session Update #73 (計分規格落地：分池、門檻 30/7、冷啟動不出數字)

founder 給了一整份 1-99 個人化百分位的規格，外加 Snapshot 動效、雙環數字、
Garmin 對齊。三個分岔他當場拍板：維持 `Edge Score` 命名（TEI 不解禁）、
現在就存序列先算 z、冷啟動不出數字。

## 一、他的規格裡有一點直接推翻我前一刀的設計

> 不能將 2 秒臉部掃描，與 60 秒深度掃描放在同一個百分位池裡比較。

#72 我才把 enrollment 的 3.6s 與日常掃描的 ~10s 併進同一個 seed，而且把
「3.6s 比較短、噪音較大」寫成一個**可以接受的限制**。那是寫錯的：短窗平均本來
就比長窗離散，混池會**系統性灌大分母 → 分數全體往 50 收，而且看起來完全正常**。
比明顯壞掉危險得多。已在 EDGE-SCORE-DEFINITION §5.1 加更正段。

## 二、我擋掉了他規格裡的一格，理由記在文件

他畫的冷啟動畫面是「TEI 57 · 初步估計」。沒有做 —— 那個數字跟成熟期的百分位
**在視覺上是同一個東西**，而使用者讀數字不讀小字；而且第 1 天 57、第 30 天 82
是一次假的變化。這與**他自己訂的第 1、3 條規則**衝突（規則是自洽的，只有那個
畫面不自洽）。替代方案：進度環 + 帶位詞 + 納入清單，三樣都是真的。

## 三、`TEI` 沒有跟著 `PR99` 一起解禁，理由不一樣

`PR99` 上次查證後是純品牌衛生問題，數學沒錯，所以解得掉。`TEI` 不行：repo 裡有
兩個活著的 `legacy/tei.ts`，解禁之後「TEI」指新主分數還是那個 retired layer 會
變成永久歧義。而全名 "Tenki **Emotion** Index" 撞 North Star §7（App Store 4.2）
—— 產品不量情緒，量的是臉部特徵點穩定度。

## 四、這一刀最值得記的三個方法學教訓（都是自己踩的）

🔴 **「值對」與「看得見」是兩件事。** 進度環被 `.tl-edge svg{display:none}`
（給 legacy SVG 寫的規則）整個關掉，而 dashoffset 與文字**全部正確** ——
我那組只驗值的斷言全綠。後來 `getBoundingClientRect()` 回 0 才抓到。

🔴 **週期性不能拿 `t % LOOP` 去驗。** 我第一版寫「t 與 t+60000 產出相同值」——
那個等式因為 `ph` 就是用 `t % LOOP` 算的而**恆成立**，連壞掉的乘數 0.23 都通過。
真正的性質是**接縫連續**。

🔴 **git stash 對照要確認對照組真的跑了。** 我 stash 之後跑截圖腳本、腳本因為
找不到我的新元素而拋錯沒產圖，我卻複製了上一輪的舊檔案當「對照」—— 等於拿檔案
跟自己比。第二次改用不碰新元素的探針才是真的對照。

## 五、實走回饋（同日稍晚）：第一組再現性數字 + 同一個錯的第二個實例

founder 跑了自測：**0.886 / 0.912 / 0.876，同場標準差 0.0186，全部 tier B**。

🔴 **那個「全部 tier B」揭發了我剛修過的同一個錯。** tier A 讀 landmark 位移、
tier B 讀整幀 luma 差分，`buildEvidence` 的註解自己寫著「平均起來哪個都不是」
—— 它在**一次掃描之內**守住了，跨掃描的百分位池卻沒有人守。
**教訓不是「再分一次池」**，是：每次寫下「這兩個量不可比」，要立刻問它在哪幾層
成立（一次量測之內／跨掃描的池／跨裝置／跨版本）。守住一層不代表守住其他層。

⚠️ 0.0186 只是分母的一半。跨天 SD 若只有 0.02，量到的變異有 **86% 是儀器噪音**，
分數等於隨機。而且三次都擠在 0.876–0.912（尺度頂端），動態範圍本來就窄。

他也退回了斜紋底（「視覺儘量採用原本的」）—— 誠實度的加重改由文案承擔。
另外我環下方那行進度把 Snapshot 推低 36px，已併進既有狀態行還原。

## 六、下次接手點

- 🔴 **跨天資料還沒有** —— 要 founder 隔天再跑一次自測，才知道 z 模型成不成立。
- `weightedPercentile()`（Weibull 分母 +1）已寫好測好，**故意還沒接上**。
- `apps/mobile` 的 `deriveDailyEdgeScore` 仍是 `Math.sin(day)` + confidence 當分數。
- takeover 死碼還沒清。

---

# 2026-08-12 Session Update #72 (臉部基線種下第一筆 —— 以及差點默默毀掉 z 模型)

founder：「Preview 不是有建立臉部基線嗎？建立臉部基線卻沒有讀數好像怪怪的」。

## 一、他的直覺對了一半，錯的那半是他自己拍板的

「基線建立不給分數」是 `SOUL-SCAN-NORTH-STAR.md` §3 第 6 步的明文設計（2026-06-12）。
而且那個決定**現在比當初更站得住**：enrollment 的三個階段**任務不同** ——
**arc 階段要求使用者轉頭**（motion 門檻從 0.36 放寬到 0.62）。
拿整場去算「狀態讀數」＝ 把「你有沒有照指示做」當成「你今天怎麼樣」。

## 二、但他指到了一個真的洞，而且比他講的嚴重

🔴 **enrollment 是這個產品品質最高的一次量測**（最長、閘門最嚴、多角度、
有 blendshapes 與頭部姿態矩陣）—— 而它只存了眨眼節奏四個數字，其餘全部丟掉。
每一幀算的七項加權 `sampleConfidence` 最後只變成一個 'strong'/'steady' 的字。

而這現在特別痛：Edge Score 需要 `{mean, std, n}`，`MIN_SAMPLES_FOR_SCORE = 14`
表示要等兩週 —— **最理想的第一筆樣本卻被丟掉了。**

## 三、🔴 動手前抓到一個會「默默」毀掉整件事的陷阱

兩支算的 stillness **不是同一個量**：

| | 中心 | 位移速度 |
|---|---|---|
| readiness-scan | **bbox 中心** | `1 − speed/0.35` |
| soul-enroll | **質心** | 完全沒有，只有整幀 luma 差分 |

`readiness-scan.js:62-66` 自己的註解早就警告過這件事。

**Edge Score 的分母是使用者自己的標準差。基線用一種尺度建、讀數用另一種尺度量
→ 分數看起來完全正常而默默是錯的。** 那比明顯壞掉危險得多。

所以先抽出 `apps/preview/face-stillness.js` 當唯一來源，兩支都吃它
（readiness-scan 委派，115 條 harness 確認行為沒變）。

## 四、做了什麼

- neutral 段（且只有 neutral）餵共用 tracker → `tenki.baseline.seed.v1`
- composite = stillness 本身（眨眼基線正在建立中，沒得用）——
  與 `baseline-score.ts` 的 `blinkCadence === null` 分支一致
- 幀數 < 8 不寫；寫不成整行不顯示
- 收尾多一行「基線已開始 · 第 1 筆」

⚠️ 誠實限制：**一筆**樣本（3.6s 內的幀高度自相關，切多筆是假獨立性，
會灌爆 z 的分母）、比日常 10s 短、**不會讓人提早看到分數**。

## 五、下次接手點

- strip-color 70/70、scan-stardust 115/115；兩組反向驗證都紅過
- 🔴 **種子目前有寫入者、沒有更新者** —— 日常掃描還沒把後續樣本併進去。下一刀。
- ⬜ founder 還沒跑 `/preview/reliability.html`（再現性數字仍未知）

---

# 2026-08-11 Session Update #71 (準度：從來沒人量過 —— 建了量測工具)

founder：「大星塵目前是有真引擎的嗎？如果有它的準度如何？」

## 一、先分清楚是哪一顆

- `/preview/` hero gate 那顆 —— **沒有引擎，刻意的**。只吃 `Date.now()`
  （42 秒一圈），那個畫面相機還沒開，沒有東西可量。按下 Start 就 destroy。
- `/v3/?from=baseline` —— **是真引擎**（本 session 剛接的 ceremony + readiness）。
...
| | 現況 |
|---|---|
| 量測本身（幾何/像素統計） | 確定性計算，沒有「準不準」的問題。但有 tier 問題：追不到臉退 Tier B，stillness 從「你的臉在動」變成「畫面在動」——**它自己知道**，`resolveConfidence` 把 Tier B 的信心上限壓在 moderate |
| 效度（推論的狀態對不對） | **完全沒驗證過，短期也驗不了** —— 需要 ground truth，一個都沒有 |
| 再現性 | **沒量過，但可以量，且不需要外部裝置** |

程式碼裡查不到任何 accuracy 宣稱 —— 那是對的。

## 三、🔴 為什麼再現性最急

Edge Score 是 z 分數，**分母是使用者自己的標準差**。如果同場連掃三次的離散度
跟跨天差不多大，那分數量到的就是量測噪音 —— 看起來很有意義，實際上是隨機的。
**再現性不是品質項目，它決定 Edge Score 成不成立。**

做了 `/preview/reliability.html`：3 次掃描、中間刻意放開 15 秒。
⚠️ 放開是方法學不是禮貌 —— 背靠背連掃共用姿勢與光線，離散度會**低估**真實噪音，
讓儀器看起來比實際可靠（最糟的偏誤方向）。

## 四、⚠️ 反向驗證又抓到一個洞

「沒資料時不得顯示 0」我只驗了畫面 —— 而畫面另有「有效次數不足」的閘擋著，
所以把 `stdDev` 的 `null` 改成 `0` **不會紅**，那道紀律本身沒人守。
補了五條直接驗那兩支純函式。
🔴 **同一個性質有兩層保護時，只驗外層等於沒驗內層。**

## 五、下次接手點

- strip-color harness 61/61；四組反向驗證都紅過
- ⬜ **等 founder 實走產出真數字** —— headless 沒相機沒臉，harness 只能守結構與文案
- ⬜ 拿到數字之後才決定 `MIN_SAMPLES_FOR_SCORE` 該不該還是 14（目前是估的）
- ⬜ 分數接 UI + 持久化（那四個數字仍未被存下來）
