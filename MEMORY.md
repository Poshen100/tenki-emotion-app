...
# 2026-09-24 Session Update #2 (分數答不出「為什麼這麼低」，因為三分之一的它看不見)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder：「訊號品質分數怎麼這麼低？」

看著 接觸 100%／光 100%／穩定 92%／節律 0%、分數 **54**。
**從畫面上答不出來 —— 那是畫面的問題。**

畫面四條 bar，分數**六個**分量。**灌流佔 25（跟節律並列最重）連 bar 都沒有**；
掉幀佔 7 只有一句話。32/100 看不見。

```
接觸 100%×15=15.0  光 100%×8=8.0  穩定 92%×20=18.4
節律   0%×25= 0.0  掉幀 100%×7=7.0  灌流 22%×25=5.6  ← 看不見
                                            總計 54.0
```

→ `explainQualityScore()` 六列收在「量測細節與證據」。
🔴 承重的測試是「各列加起來等於旁邊印的分數」—— 反向驗證把 points 改成不乘
權重（正是那種很合理的第二套說法），立刻紅。加總抽成 `scoreFromComponents()`，
**算分跟解釋同一段算術**。

## 順帶讀出來的：那次擷取的問題不是相機

`安靜段讀數 0/1 —— 切不出夠多段（找到 1 段，最長 60 秒）` = 整段都安靜、
沒有轉換要切。配上灌流 22%、兩通道節律都 0、紅 183（平常 200）——
**不是曝光漂移，是透過來的脈動光本來就太少。**

⚠️ 張力值得記：畫面一直說「放鬆一點手指，不要施力」，而壓太輕會讓灌流掉。
兩個方向都會壞，中間那個帶子多寬**還沒量過**。

## 教訓

1. 一個數字由 N 個東西決定、畫面只給 M 個 → 那個數字就是任意的。
2. **理由不能取代數字。**「脈搏訊號偏弱」講不出 5.6/25。
3. 解釋要跟被解釋的東西共用同一段算術。

## 下次接手點

🔴 診斷鏈現在是通的（曝光數字 → 漂移週期 → 安靜段為什麼沒救起來 → 分數怎麼來的）。
下一輪實機看「安靜段讀數」那一行就知道往哪走。

仍然懸著：**補光燈對照只有「開」4 筆**（關著做一次就有答案）；
以及上面那個「壓多輕才對」的帶子沒量過。


# 2026-09-24 Session Update (門檻卡在平原長度上；一句跟自己證據打架的文案)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## 安靜段救援為什麼沒觸發 —— 用報告的數字反推得出來

週期 **15.24 秒** → 平原約 7.6 秒。但轉換（1–1.5 秒）**加上平滑窗（±1 秒，
兩端各扣）**吃掉一大截，實測剩 **5.1–5.5 秒**，而門檻是 **5**。剛好卡住。

兩個門檻各自重新從量測推（不是「調鬆試試」）：
- 平滑窗 ±1.0 → **±0.75**：靜止手指最大 slew ±1s 0.023／±0.75s 0.029／
  ±0.5s 0.040，門檻 0.05。±0.5 只剩 1.2 倍餘裕。
- 最短段 5 → **4 秒**：估計器要 3 秒（40 bpm 兩個週期），4 秒有餘裕。

配置比較：±1.0/5 救 44%、±0.75/5 56%、**±0.75/4 75%**、±0.5/4 83%。
取 ±0.75/4。重跑 160 組：**最大誤差 2.4 bpm、偽陽性 0、乾淨擷取 0 次被切**。
🔴 放寬的是視窗不是安全性 —— 守住的仍是「三段一致」。

## 救不起來要說是「哪一種」

三種原因三種修法。報告新增一行直說。⚠️ 而且**所有段先評分再過閘** ——
原本段數不足就提前 return，於是「兩段都有脈搏」跟「兩段都沒脈搏」回報
一模一樣，而那是完全不同的訊息。

## founder 的 UX 問題：光場燈號沒亮要不要提示調整動作

查下去發現畫面當時說 **「光已經穿過來了。保持這個位置。」** 而證據列正顯示
**「光場均勻 ✗」** —— 畫面在跟自己的證據打架。

🔴 根因：`light_locking` 被兩條路徑共用（良性 fallback ＋ `camera_adapting`）。
一個狀態、一句文案、兩種情況。修法**不新增第九個狀態**（brief 就是八個），
由指令挑文案。

🔴 **而答案是：不該給姿勢指引。** 相機在自己調增益，換姿勢改不了它 ——
給指引是**假指引**且把責任推給使用者。改成說發生什麼事＋明說不是他的手，
而真正能做的那件事（關補光燈）在就位畫面切換鈕旁邊講，那裡才按得到。

## 教訓

1. **門檻要對著它實際會遇到的量訂，不是只對理論值。** 下限 5 秒的推導是對的，
   但沒人算過裝置的平原實際剩幾秒。
2. **每個預處理視窗的代價常落在別的地方。** 平滑窗的副作用（兩端各扣）才是
   卡住實機的東西。
3. **一個狀態被兩條路徑到達時，它的文案只能對其中一條。**
4. **不是每個「燈沒亮」都該配一句調整動作。**

## 下次接手點

🔴 實機再跑一次，看兩件事：
- 讀數底下有沒有出現「從中間 N 段沒有被打擾的時間讀出來的」
- 沒有的話，報告「**安靜段讀數**」那一行會直接說是哪一種救不起來

**補光燈對照仍然只有「開」（3 筆）** —— 那條路成立的話比分段讀數更根本，
還是值得花一次「關著做」。


# 2026-09-21 Session Update (脈搏一直都在，在相機沒有打擾的那幾段裡)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## 兩個數字兜不攏，那個矛盾就是答案

報告終於給出週期 **17.14 秒**，而同一段裡最大單秒跳動是 **22%**。
17 秒的振盪一秒內走不了 22% —— 兩者同時成立只有一種形狀：
**停住、跳一下、再停住**。合成驗證（±16%／17 秒／0.5 秒轉換）給出
0.2922／0.2150／比值 0.74，對上實機 0.2944／0.2201／0.75。

## 重點是結構性的，不是頻譜性的

**大部分時間相機是安靜的。** 60 秒裡 7 次轉換只打擾約 4 秒。而整段一起分析
會把剩下 56 秒全丟掉，因為轉換是寬頻的、正好落在心搏帶 —— 這也是 §22 濾波
救不了的原因：要除的跟要留的在同一頻帶。

實測：**整段被拒答，裡面一段 6.8 秒的平原讀出 67 bpm／節律 0.93**（真值 68）。

→ 新模組 `quiet-segments.ts`：**排除**被打擾的段落，不動訊號。
安全性靠**三段互相獨立且一致（≤6 bpm）**，不靠單一視窗 —— 短視窗更容易碰巧
看起來週期。掃 448 組：**最大誤差 2.4 bpm、偽陽性 0**，只救起 203/448
（拒答比接受多，那是對的姿態）。

## 三個這次學到的東西

1. 🔴 **門檻要從量測的間隙取。** 第一版拿心搏起伏推（AC/DC 0.5–2% → 2%/秒），
   **錯的** —— 競爭者不是脈搏是**呼吸基線起伏**。實測靜止手指最大 slew 0.023、
   裝置轉換 0.10–0.16。2% 把乾淨擷取切成三段（真的發生了），改 0.05。
2. 🔴 **假影會關掉防著它自己的閘門。** 漂移把整段灌流從 0.006 灌到 0.148，
   於是 `lowPerfusion` 被推過門檻、救成 67 bpm。**閘門要量在證據來的地方** ——
   段落是證據，灌流就在段落裡重算。
3. 🔴 **承重的那道防線要指得出來。** analyze 裡「只有節律才嘗試」那個合言
   拆掉不會弄紅任何測試（per-segment 灌流已獨立擋住；品質低於門檻卻還能分段
   一致的 fixture 我建不出來）。註解照實寫成縱深防禦，不吹成承重。

## 誠實的代價全部明寫

PRV／呼吸／拍形一律 null（切口兩邊的拍不相鄰）；品質分數不修補；
畫面要說出用了幾秒（旁邊「實際時長 60s」是真的但會誤導）；
`PpgSignalQuality.fromQuietSegments` —— 否則片段讀數會**直接變成被接受的
anchor 餵進脈搏基線**，而它是同一個量但更吵（2.4 vs 1 bpm）。
旗標不取消資格，它讓「要不要降權」之後還做得出來（founder 決定）。

## 下次接手點

🔴 **實機零驗證。** 448 組是合成的。下一次實走要看的是：
讀數底下有沒有出現「從中間 N 段沒有被打擾的時間讀出來的」。
- 出現了 → 拿血氧儀／手錶對照，|Δ| ≤3 bpm（§12 第 4 條）
- 沒出現 → 看段數，可能轉換太慢或太頻繁

另外仍然懸著：**補光燈開／關的對照還是只有「開」1 筆**（§12 第 23 條）——
那條路如果成立，是完全在我們手上的解，比分段讀數更根本。
以及 #6 那一次「lock 出現但最終沒有讀數」。
