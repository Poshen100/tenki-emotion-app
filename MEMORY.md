...
我提議「沒有比對到就印『—』」，founder 當場否決：**「印『—』這樣看起來會像壞掉」**。
他是對的 —— 破折號只是把空白換一個寫法，欄頭仍然在承諾一個它給不出來的值。

改法：第三欄**整欄不渲染**，並補一句「這則快訊沒有指名結構 —— 三個流程都可以選」。

⚠️ 更重要的是**根因**：比對只讀 `strategyHint`，而 demo 按鈕寫死 `'Mancini'`，
所以「比對不到」那一格在 demo 路徑上**根本走不到**，看起來一直是好的。
真實 TradingView 快訊把結構寫在訊息欄或條件欄（founder 那則是「ES1! 下穿 7,740.00」）。
→ harness 因此改走 **live 路徑**（塞 `tenki.alert.channel` + `page.route` 攔 `/api/alerts`）。
那是唯一能餵出「demo 造不出來的形狀」的辦法。

⚠️ 關鍵字只收**指名結構**的詞，不收方向詞。「下穿」只說了價格穿過一個數字，
沒說這是不是一個 failed breakdown —— 那正是使用者要守望的事。

## 2. 信心低卻上 gold

founder 的截圖：穩定度 58% / 未偵測到眨眼，教練文案寫著「讀數僅供參考」，
而外框與完成鈕是 SECURED 的金色。

> **顏色宣稱的比文字強。** 兩個一起出現時，使用者信的是顏色。

規則落成 `securedEarned(reading)`，**兩個出口都要問它** —— 外框／完成鈕吃 `.secured`，
星塵吃 `revealTone()` 的第二個參數。只擋一邊的話文字說僅供參考、星塵還閃 SECURED。

## 🔴 這一輪自己踩到的：守門放在一支不會被跑到的 harness 裡

gold 的真值表與接線守衛第一版寫進 `preview-scan-stardust.mjs`，手跑全綠 ——
而那支**不在 `verify.sh` 也不在 CI**（檔頭寫明它倚賴「容器連不到 cdnjs」，CI 連得到，
前提整個反過來）。搬到 `preview-scan-blink.mjs`（同樣只要把模組載進一個頁面，兩邊都跑得到）。

**一條不會被執行的斷言，跟一條永遠不會紅的斷言是同一種東西。**
這是這條分支上同一個家族的第四次，已進 PLAYBOOK §6。

## 下次接手點

- PR #260（分支同名）現在有 4 顆 commit，`npm run verify -- --quick` 全綠。
- **preview 視覺類改動仍需 founder 手機實走**：模板表兩種狀態、低信心掃描結果不再是金的。
- 低信心 → 不上 gold 這條，**harness 只驗到純函式 + 接線**；
  「真的跑一場低信心掃描」在容器裡造不出來（光線來自 stub video），那一格靠實走。
