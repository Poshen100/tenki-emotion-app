...
# 2026-09-17 Session Update #3 (實機第五次：增益修正這條路關了，改問「別讓 AE 有東西追」)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## 我要的數字沒拿到，因為我自己的 bug

報告印出「慢速擺動週期中位數 **NaN 秒**」，後面還接著最樂觀的那句
「比一次心搏慢 = 原理上可以除掉」。

🔴 **`!== null` 對 `undefined` 是 true —— 這是 `86b39915` 同一個 bug、
同一個檔案，而且是它的教訓寫進 PLAYBOOK 三個 commit 之後。**
`driftPeriodSec` 是同一天加的欄位，舊紀錄在那裡都是 undefined。

🔴 第二個錯更值得記：**NaN 打敗了 null 檢查，也打敗了兩個 `<` 比較**
（NaN 的比較全是 false），於是**靠著一路失敗**落進最鼓舞人心的分支。
→ 規則：**樂觀的結論要靠通過一個正向測試拿到，不能靠通不過別的測試拿到。**

修四層：`filter(present)`／`medianOf` 濾非有限值／`fmt` 不印非有限值／
`driftPeriodNote` 改正向條件。實測四層同時拆掉才紅，任一層留著都夠。

## stabiliseGain 這條路關了（函式已刪）

原本要做成有條件的（`driftPeriodSec ≥ 4` 才套用）。橫掃之後否證兩次：

1. **週期慢不代表除得掉。** 階梯 ±15%／2 秒的基頻是 4.07 秒（門會開），
   修正後誤差 **27 bpm**（真值 68 → 41），而原始擷取本來是**拒答**。
   帶內的是**諧波**不是基頻，移動平均除不掉諧波。
2. **折疊讓快的報成慢的並過門。** 0.8 秒 → 4.07 秒、1.2 秒 → 6.21 秒。
   我先前只量了 1.4 秒**一個點**就以為 4 秒安全。**一個點不是一條曲線。**

真正該看的量（帶內漂移能量）**早就在量了** —— `perfusionIndex`，也就是
run 2 那個「血流訊號強＋找不到節律」的矛盾，而修正不會移除它（0.148→0.141）。

→ 函式、常數、11 條測試全刪。量測留在 §20／§22。
**不能上線的程式不該留在引擎裡當候選** —— 兩個 session 各花一輪論證它。

## 還剩的那條路：不要給 AE 東西去追

報告把曝光鎖講死了：**8/8 次 `exposureMode` 根本沒有 manual 可用。**
所以剩下的不是修正漂移，是讓漂移不要發生。而補光燈貼著指尖正是 AE 最難
平衡的場景（實機紅 **200.95**、綠 **37.45**）。

做成一個裝置自己答得出來的對照：就位畫面可關補光燈、`torchOn` 落地、
報告新增「補光燈對照」把開／關的擺動並排。三件刻意的事：
一邊不算對照（直說「再做一次才比得出來」）；舊紀錄 undefined 要說「沒紀錄」
不得當成「關」（同形狀第三次，這次一開始就明寫 `=== true`/`=== false`）；
擷取途中按不到切換鈕（中途換光源那筆兩邊都不算）。

## 下次接手點

🔴 **兩次擷取，一次開補光燈一次關**，然後看報告「補光燈對照」那一行：
- 關掉之後擺動掉下來 → **補光燈就是原因**，關掉即可（完全在我們手上）
- 沒掉 → 不是補光燈，要走**原生曝光控制**（等 Mac）

另外仍然懸著：#6 那一次「lock 出現但最終沒有讀數」（灌流條件是 `e871539b`
才進 live gate 的，那批紀錄可能混著修之前的 —— 清掉 log 重跑一輪就分得出來）。


# 2026-09-17 Session Update #2 (實機第四次：自相關把自己的搜尋邊界當成心率回報)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 給了第一份真的驗收報告（9 次擷取）

`86b39915` 修好之後終於拿到曝光數字：**DC 慢速擺動 0.2879**、單秒跳動 0.1782、
可疑 6/7、fps 59.91、曝光鎖「成功 7/7（whiteBalanceMode）」、
#6 Pulse Lock 誠實性 **✗ 1 次 lock 出現但最終沒有讀數**。

## 從比值反推漂移形狀，然後發現了更重要的東西

step/drift = **0.62**。方波會讓這個比值接近 1.0（相鄰一秒桶跳完整幅），
0.62 對應**約 5 秒的平滑漂移**。合成重現：T=5.0／±15.5% → drift 0.2893、
step 0.1829，裝置簽章對上了。

把那個形狀餵進未修的 pipeline：**回報 200 bpm、節律 0.75，真值 68。**
過了每一道閘，錯 130 bpm。200 就是 `MAX_PLAUSIBLE_BPM` 換算的 lag ——
**自相關搜尋範圍的牆，不是量測**。`MIN_PERIODICITY` 擋不住，因為慢就是平滑，
平滑在每個短 lag 上都自相關得很好。

→ `dominantPeriod` 現在拒絕落在搜尋邊界上的 lag。
🔴 **兩個 lag 都要檢查，真正會觸發的是第二個**：`preferFundamental` 修八度
誤差是換到**更短的** lag，能把內部 argmax 搬到牆上。只檢查 argmax 的半修版
實測仍放過 200 bpm／節律 0.53。代價明寫：可回報範圍變 **40.9–180 bpm**。

## 報告有三行講了資料沒支持的事（而 founder 正拿它們指揮我）

1. 🔴 **「曝光鎖 成功 7/7（whiteBalanceMode）」—— 曝光從來沒被鎖過。**
   `applied` 是整個 applyConstraints 的單一布林，而只有 whiteBalanceMode 被
   要求。報告替七次「完全沒鎖曝光」印出「成功」，還把剩下的擺動解釋成
   「瀏覽器收了約束沒真鎖」。兩半都錯，且把下一步指向錯的問題。
2. 🔴 **通道註腳是無條件印的** → 印在「紅 201、紅的節律比綠高」上面，
   斷言「補光燈把紅通道打飽和」。飽和是合言，兩個條件都要成立。
3. 🔴 **「拍形穩定度 最低/中位數/最高 0.96」只有一筆資料。** `Spread` 現在帶 count。

## driftPeriodSec：直接量漂移週期（把反推換成量測）

三個坑：① `dominantPeriod` 是錯的工具（按 overlap 正規化偏好長 lag，在 5 秒
漂移上把 argmax 送到 **lag 25** 且 periodicity 1.000）→ 改用平均穿越計數；
② **斜坡會長出週期**（穿越平均一次＋呼吸 → 自信的 19.3 秒）→ 要求兩個完整
週期；③ **快的漂移讀成慢的**（真 1.4 秒 → 3.58 秒），錯的方向最危險 →
另立 `DRIFT_PERIOD_TRUSTWORTHY_SEC = 4`。

🔴 乾淨擷取回報 4.4 秒，那是**呼吸**，不是誤差。**週期只說形狀，幅度才說來源。**

## stabiliseGain：§20 的結論錯了一半

加了邊界閘門之後，在實機形狀上：未穩定化 **拒絕** → 穩定化 **69 bpm／0.83**
（真值 68）。所以 §20「最可能那格救不了」是拿方波在推平滑漂移的結論。
**仍然不接**，但理由只剩兩條：形狀還是反推的、以及 ±35%／1.4 秒那格會產出
過閘但差 9% 的讀數（§12 第 4 條要 ≤3 bpm）。接線條件收斂成一條：
只在 `driftPeriodSec ≥ 4` 且 `dcDriftFraction ≥ 0.05` 時套用。

## 教訓

1. **有界搜尋的 argmax 停在邊界上，不是量測，是那個界。**
2. **拿 A 形狀的量測推 B 形狀的結論，等於沒量。**
3. **上限／門檻類斷言的 fixture 必須是最長／最壞形。** 報告長度上限 `< 24`
   自從加了曝光段就一直是空的（fixture 的 exposure 是 null，整段塌成一行），
   實測完整報告 26 行。

## 下次接手點

🔴 **報告的「慢速擺動週期中位數」** —— 一次正常擷取就有。≥4 秒 → 接
`stabiliseGain`（條件化）；<4 秒 → 是心搏同頻的干擾，濾波無效，要原生曝光控制。
另外 **#6 那一次 lock 說謊要追**：灌流條件是這次才進 live gate 的（`e871539b`），
那 9 次裡可能混著修之前的紀錄 —— 清掉 log 重跑一輪就能分辨。


# 2026-09-17 Session Update (三次實機拿不到曝光數字：報告自己被舊紀錄炸掉；另外灌流從來不是心率的閘門)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 的一句話

第三次實機：「**節律 很難測**」。畫面：接觸 68%／光 73%／穩定 84%／**節律 0%**，
理由列 ✓血流訊號強 ✓手很穩 ✓覆蓋完整 ！找不到穩定的脈搏節律，
擷取中三格證據列的「**光場均勻 ✗**」—— 也就是 §19 的 `slowDriftDominates === true`
在實機上真的有在燒。

## 這次最重要的發現：報告一直是壞的

🔴 **驗收報告的「實機驗收」區塊三次實機都顯示「—」，不是資料不足，是它炸了。**
schema migration 用 `c.exposure !== null` 判斷欄位在不在 —— 而舊版 localStorage
紀錄那個欄位是 `undefined`，`undefined !== null` 是 **true**，於是往下拿
`c.exposure.dcDriftFraction` 直接爆。2026-09-12 加欄位那天壞的。

⚠️ **harness 完全看不到**，因為它每次都先清空 log，永遠只有新格式的紀錄。
修法 `present()` 同時判 null 與 undefined，補了三條「舊紀錄」測試。commit `86b39915`。

**所以：三次實機、三次都沒有任何曝光數字回來，而那三個數字正是現在唯一缺的東西。**

## 順手炸出來的兩個既有 bug（都修了）

想濾掉曝光漂移（`stabiliseGain`，見下）接線那一刻，兩個一直存在的 bug 現形：

1. 🔴 **灌流從來不是心率的硬閘門。** 心率只看品質分數與節律；弱脈搏通常同時很吵，
   節律會先擋下來，所以「看起來」有灌流閘門。把噪音也降下來兩者就分開 ——
   新 fixture `quietWeakPulse` 品質 **74**、節律分量 **1.00**、灌流分量 **0.00**，
   拆掉閘門實測報出 **68 bpm**：一份裡面沒有血流訊號的心率。
2. 🔴 **live gate 少引一個條件就會樂觀。** 新條件只加進 `analyze.ts` 的那一刻，
   Pulse Lock 就開始對一個 pipeline 最後會拒絕的擷取承諾讀數 —— 正是 `live.ts`
   存在要防的方向。**任何加進心率閘門的條件都必須兩邊都加，否則鎖會說謊。**

## stabiliseGain：量過、有 11 條測試、**刻意不接線**

增益是乘性的，`x/avg` 會把它消掉 —— 是恆等式不是啟發式（±30% 正弦增益前後差
30.8% → 穩定化後 0.35%）。但**不接**，三個理由：

- 裝置上效益未驗證（表是合成的，而實機 `dcDriftFraction` 因為上面那個 bug 拿不到）
- 最可能的那一格救不了：±20%／3.0 秒 節律 0.000 → 0.104，**仍然拒絕**
- ±50%／0.8 秒 = 1.25 Hz 在心搏帶內，讀起來是週期的（0.708）並給出自信的
  **40 bpm，真值接近 69** —— 比「救不了」更糟

接線的唯一條件寫在 `docs/PHONE-PPG.md` §20。驗收清單新增第 21 條。

## 教訓

1. **量一個候選修法，要連「它救不了哪一格」一起量。** 只量救得起來的那格，
   就會把部分解當成解接上去 —— 這條鏈上部分解的代價是使用者拿到一個自信的
   錯讀數，比拒絕更貴。
2. **harness 清空狀態 = harness 永遠看不到 migration bug。** 任何讀持久化資料的
   程式，測試要餵「舊格式的紀錄」，不只餵自己剛寫的那筆。
3. `!== null` 不是「欄位存在」。（已是 PLAYBOOK 級候選，第二次出現就提煉。）

## 下次接手點

🔴 **只缺一件事：founder 再跑一次 `/finger/`，把驗收報告的「曝光」段貼回來。**
三個數字指三個不同方向：
- 漂移 ≫5% 且 `exposureLock.applied === false` → 瀏覽器不給鎖，要原生
- 漂移 ≫5% 且 `applied === true` → 收下約束卻照樣調，鎖這條路沒用；看擺動週期
  決定要不要接 §20 的濾波
- 漂移 <5% → **§19 的診斷是錯的**，要改看時基與估計器

其他仍欠 founder 的：即時候選拍點偵測器（在它存在前不得有漣漪／觸覺回饋）、
PULSE LENS 的七份交付文件（before/after UX map、visual state map、copy table、
timing model、test plan、QA steps、reality map）、§12 驗收清單 21 條一條都沒跑。


# 2026-09-15 Session Update (覆蓋地圖做成連續的場：像熱像儀的是成像方式，不是彩虹)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 要的

看到熱像儀抓漏的廣告，問「手指覆蓋的視覺反饋是不是也能用類似紅外線成相的效果」。

## 答案：成像方式可以抄，彩虹不行

**彩虹不能用，兩個各自成立的理由：**
1. 彩虹裡每個顏色在這個產品裡**都已經有主人** —— 綠 = success、青 = Clear
   帶位／ACTIVE、紫 = Premium、燒橙 = Strain 帶位、紅 = error、金 = SECURED。
   一條彩虹等於同時亮起六個不相干的宣稱。（跟 CLAUDE.md 動畫段那條
   「產生新顏色之前先問這個顏色是不是已經有主人」同一條規則。）
2. 彩虹的亮度不單調，本來就是表示「量」的爛編碼。表示量的正解是
   **單一色相、亮度單調的 sequential ramp**。

→ 用 repo 自己量過的琥珀階（2.5→4.2→6.3→8.0→9.9:1，嚴格遞增）。

## 解析度與渲染是綁在一起的

格子 4×4 → 8×8（每格 8×8 = 64 px，SE 約 6.3%，5 幀環平均後約 2.8%）。
🔴 這**只有在渲染連續時才誠實**：二值化下這種雜訊會讓邊界格在兩個顏色之間
閃爍，看起來像壞掉；連續渲染下同樣的雜訊只是一點點斑駁。
⚠️ 要再提高格數，先確認畫它的東西仍然是連續的。

⚠️ 刻意不做雙線性內插 —— 會讓 8×8 看起來像高解析度熱像，而我們不知道缺口
精確在哪個像素。保留方格反而誠實（便宜的熱像儀本來就長這樣）。

⚠️ 文案加「也**不是溫度**」：熱像的視覺語言會讓人以為手機在量溫度。
**顏色會宣稱事實，構圖也會。**

## 測試

harness 107 → 112，兩條直接守界線：整張圖只准一個色相（換 FLIR 彩虹就紅）、
色階必須連續（改回二值就紅）。兩個破壞都驗過。
測試的格子索引改成 grid-relative —— 原本寫死 `[0,1,2,3]` 叫「第一列」，
只在 grid=4 時成立，改成 8 會**悄悄變成別的意思**。

## 下次接手點

1. **founder 實走 `/finger/`**，這次要看三件事：
   (a) 曝光那一段報告（§19 的三種結果各指向不同方向 —— 見上一條紀錄）；
   (b) 覆蓋地圖在真手指上讀不讀得懂；
   (c) 驗收清單第 20 條：指腹往已知方向挪開，看亮的是哪一列（對應關係穩定
       才能把文案從「看著圖移動」改成「往上移」）。
2. 驗收清單 20 條，一條都還沒跑。

# 2026-09-12 Session Update (🔴 實機第二次：不是通道飽和，是相機在自己調曝光)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 實機回報兩件事

1. 「一條橫杠，使用者的感受可能會覺得壞掉了。」（前 15 秒中央畫 `—`）
2. 「找不到穩定的脈搏節律。（跑一半了仍然是零%）」

## 🔴 第二件把我上一個診斷否決了

第二次擷取：接觸 100%、**光 100%**、穩定 83%、節律 **0%**。
光 100% = 幾乎沒有像素被削波 → **紅通道沒有飽和**。§16 的通道修正解釋得了
第一次（節律 8–9%），**解釋不了第二次**。已在 §16 加更正段落，並把
「看到節律讀不到就預設是通道問題」列進 CLAUDE.md 禁止事項。

## 真正的線索：理由列同時出現 strong_pulse 與 irregular_periodicity

`perfusionIndex` 量的是**帶通後的 AC/DC** —— 所以 `strong_pulse` 不是「脈搏
讀得到」，而是「心搏頻帶裡有很多能量」。加上節律 0 = **那些能量不重複**。
真的沒脈搏的手指會讀到**低**灌流。

主要嫌疑：auto-exposure / AWB 對著被補光燈照亮的指腹持續重調增益 —— 大幅、
階梯狀、不規律，落在心搏頻帶、蓋掉心搏起伏、沒有週期。
**合成重現成功**：疊上階梯式增益擾動後灌流上升、節律崩掉，跟畫面上那一對一樣。

## 做了什麼

- `ppg/exposure-stability.ts`：DC 慢速擺動（一秒桶 p95−p5 / DC 中位數）＋ 時基。
  `DC_DRIFT_SUSPECT = 0.05` 是從「心搏 AC/DC 約 0.5–2%」推出來的，不是挑的。
  乾淨合成擷取實測 1.4%。**只標記可疑，不下判決。**
- `lockExposure()`：best-effort 鎖曝光/白平衡/對焦。🔴 鎖在**擷取開始的那一刻**
  —— 就位閘保證手指已經在鏡頭上、AE 已收斂。那道閘意外的第二個用處。
  `exposureLock.applied` 與實際擺動分開記：瀏覽器可以收了約束照樣繼續調。
- 掃描畫面直接顯示擺動與 fps（第 15 秒就算得出來，不用等 90 秒）。
- 橫槓 → 「累積中」，跟「節律」那一列同一個字。
- `strong_pulse` 文案「脈搏清楚」→「血流訊號強」（舊的因為這次的發現變成錯的）。
- 驗收報告新增「曝光」段；驗收清單仍是 20 條。

## 教訓（已提煉進 PLAYBOOK）

我自己寫壞一條斷言：右邊是 `Number.parseInt(三元式兩邊都回 '100')`，等於
「節律 < 100」恆真。**新問法：右邊那個數字是從哪裡來的？** 不是另一次量測、
而是手打常數或恆定式子 → 大概沒在守東西。

## 下次接手點

1. **founder 再實走一次，然後貼「曝光」那一段報告。** 三種結果三個方向：
   - 擺動 ≫ 5% 且鎖失敗 → 診斷成立，但 Web 擋不住 → 要原生（VisionCamera）。
   - 擺動 ≫ 5% 且鎖成功 → 瀏覽器收了約束沒真鎖 → 同上。
   - 擺動 < 5% → **診斷錯了**，換方向查時基（fps / 最長間隔）與估計器。
2. 驗收清單 20 條，一條都還沒跑。
3. harness 107 passed。

# 2026-09-12 Session Update (覆蓋地圖：缺口在「哪裡」)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 要的

「幫我用先前版本的視覺設計，將手指沒有遮到的地方做視覺化反饋，這樣可以確保手指完全覆蓋。」

## 做了什麼

`packages/engine/src/biometric/ppg/coverage-map.ts`（4×4）＋ `/finger/` 就位畫面。
沿用舊 onboarding 的 target ring 構圖，**井裡不放相機影像** —— 蓋好的鏡頭是一整片
均勻的紅，缺口恰好是影像上唯一看不出來的東西。詳見 `docs/PHONE-PPG.md` §18。

## 🔴 三個被實測／截圖改掉的決定

1. **不講方向。** 第一版文案是「往上移一點」。地圖的軸是**影像座標**，後鏡頭影像
   上緣對應到手機哪一邊取決於裝置方位與瀏覽器 orientation 處理 —— 沒驗過就叫
   使用者往錯的方向移動比什麼都不說更糟。改成講跟方位無關的（邊緣 vs 中間、
   一邊 vs 一個角）並導向那張即時圖。**驗收清單新增第 20 條**專門驗這個對應，
   驗過才能加方向；另有一條斷言擋方向詞。
2. **斷言的測資太乾淨。** 「地圖 = 閘門同一個數字」用對齊格線的留白驗，而對齊時
   兩個公式剛好同值（0.75 = 0.75），換公式照樣綠。半格高才分得開。
   → 已提煉進 PLAYBOOK（同一條規則的第四次，新增「測資能不能分開兩個公式」這一問）。
3. **畫面自相矛盾。** 閘門看 1.5 秒窗口、地圖看現在，於是「維持住，不要動」印在
   一個看得見的缺口旁邊。**截圖抓到的，不是斷言。** 教練句改成跟著地圖走。

## 順手拆的東西

取樣器的像素化簡拆成 `reduceRoi(data, ts)` —— 像素 → 格子的索引換算**沒有任何
合成 fixture 碰得到**（合成器產出的是已化簡的 `PpgFrame`）。harness 現在直接餵
已知圖樣的 `Uint8ClampedArray`，斷言「整條上緣 → 剛好第一列」「整條左緣 → 剛好
第一欄」；row/col 對調會同時打紅兩條。

## 下次接手點

1. **founder 實走 `/finger/`**：除了原本要看的（接觸/光/穩定真值、閃光燈 advisory、
   節律是否終於立住），多一項 —— **第 20 條**：指腹往已知方向挪開，看亮的是哪一列。
2. 驗收清單現在 20 條，一條都還沒跑。
3. harness 100 passed。verify.sh EXIT=0。

# 2026-09-12 Session Update (就位閘：沿用舊 onboarding 的門，不沿用它的實作)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 問的

「我們原本做好的手指建立流程，視覺介面是不是可以沿用？是不是可以整合兩個的優點？」

## 回答與做法

舊流程 = `/preview/`（`apps/preview/index.html` + `baseline-onboarding.js`）。
真正值錢的是 **pre-roll**：`對到鏡頭 → 蓋滿邊緣 → 穩住 1 秒 → 開始基線`
—— 手指沒就位就不開始計時。`/finger/` 是一按就起跑 90 秒，實機第一次因此
燒掉兩次完整擷取。

**沿用那道閘，不沿用那份實作**，兩個理由都不是美感：
1. `apps/preview/camera-scan.js`（320 行）是**第二套手寫 PPG** —— 自己做
   peak 偵測、自己算 `hrv`/`rr`。`/finger/` 整個架構（bundle 由
   `packages/engine` 產生、`--check` 卡在 merge gate）就是為了消滅鏡射漂移。
2. `index.html` 把相機輸出標成 HRV（`ti-hrv:354`、`metric-hrv:452`、
   「能提升 HRV 精度」:489）。違反 founder 2026-09-11 第 1、2 條。舊流程是
   在那條規則之前寫的。

新增 `packages/engine/src/biometric/ppg/capture-readiness.ts`，`/finger/` 多一個
`ready` phase。按鈕從「開始校準」改成「打開相機」。

## 🔴 這次最重要的一件事：我的第一版設計被 fixture 否決了

第一版把**光與晃也寫成阻擋條件**。量完之後：

| fixture   | 第一個窗口 | 90 秒後 |
|-----------|-----------|--------|
| `clipped` | 光 0.00（紅 255）| 有讀數，走綠通道 |
| `motion`  | 穩定 0.00 | 有讀數，走綠通道 |

擋它們＝**拒絕啟動會成功的擷取**，而 `clipped` 那條更糟：它把前一個 commit
的通道修正原地撤回。改成只擋接觸，光與晃降成 advisory（講但不擋），並用一條
測試鎖著。

⚠️ 這個錯誤**長得像是更嚴謹**。「訊號不好就不讓你開始」聽起來永遠是對的 ——
唯一的分辨方法是去量那些擷取最後有沒有讀數。

## 教訓（已提煉進 PLAYBOOK §3）

教練句在**使用者做對的時候是空字串**（`BLOCKER_COPY[null]` = undefined），
harness 只斷言那一行「看得見」——`.coach` 有 `min-height`，空字串照樣有高度，
所以綠。**截圖抓到的，不是斷言。** 同一個病的第三次（`.dimFill` 量
`style.width`、「拒答也要進紀錄」走到另一條 return）。
規則：**斷言要量使用者實際讀到的那個量**，量到容器就等於沒量。

## 下次接手點

1. **founder 實走 `/finger/`**：現在會先進就位畫面。要看的是
   (a) 接觸/光/穩定在真手指上的值，(b) advisory 有沒有講中閃光燈，
   (c) 進到 90 秒之後節律是不是終於立得起來（通道修正的第一次驗證）。
2. 驗收清單 §12 已經 19 條，一條都還沒跑。
3. §16 的第二個嫌疑（穩定 42–51%）仍刻意未動 —— 先看通道修好後會不會自己回升。

# 2026-09-12 Session Update (🔴 第一次實機：脈搏不在紅通道裡)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 的實機回報

真 iPhone、補光燈開著：**接觸 100%、光 72%、穩定 42–51%、節律 8–9%**，
兩次擷取都沒有立住脈搏。一句話：「節律不好偵測」。

節律 8% 換算回原始 periodicity ≈ 0.24（合成訊號 0.93）。

## 根因

手指壓在亮著的鏡頭上 → **紅通道被頂到感光上限，脈動成分被削掉**。綠通道離飽和
還遠，訊號還在裡面。而 pipeline 只吃紅通道。

🔴 **這件事我稽核 PR #148 時就寫下來了**：「紅通道在 torch 下常飽和，所以它同時
跑紅／綠兩條 buffer 自動選；我們目前只吃紅通道，這是實機階段要驗的事。」
記下來、標成待驗、然後實機第一次就撞上。已提煉進 PLAYBOOK：**寫下「之後要驗」
之前先問「現在做掉會不會比較便宜」**。

## 修法

`ppg/channels.ts`：兩個通道都算，選 periodicity 高的，回報**每一個**通道的
亮度／灌流／節律。🔴 不是「改用綠通道」—— 哪個通道有脈搏取決於裝置與曝光，
換一個寫死的只是把失敗搬到別支手機。平手歸紅（避免通道在兩次擷取間跳動，
那會被讀成跨天變異但其實是算術）。

合成證明：`clipped` fixture 從**拒答**變成**有讀數**。`live.test.ts` 原本把
clipped 列在「會被拒答」的名單裡，照實移出並註明。

實機報告新增「通道」段（排在編號檢查之前），直接寫出判讀規則。

## ⚠️ 刻意沒改的第二件事

靜坐時「穩定」只有 42–51%。`motion` 是逐幀紅平均差值 —— **紅飽和時那個差值主要
是削波雜訊不是手在動**。但這次失敗的原因是節律（品質 63–65 仍然過得了 45 的
門檻），穩定沒有擋住讀數。一次只驗一個修正：先看通道修好之後穩定會不會自己
回升。列為實機第 19 條。

## 下次接手點

- 🔴 **founder 再走一趟**，看報告的「通道」段：紅亮度接近 255 且節律遠低於綠
  = 診斷成立；若兩個通道節律都很低 = 另有原因，要重新查。
- 實機清單 17 → 19 條。

---

# 2026-09-12 Session Update (實機驗收儀表 —— 把 §12 的三條關鍵檢查交給裝置自己算)

⚠️ 依協議 2b：**不編號**。分支 `claude/tenki-biometric-v2-ubzosn`，PR #256。

## founder 的指令

「全依建議，依序開工」—— 指的是我上一則建議的實機驗收順序（#15 → #6 → #14）。
⚠️ 但那三條都要真手指在真手機上跑，**Claude Code 按不動**。所以「依序開工」的
可執行版本是：**把那三條從一張紙上的表，變成頁面自己會量、會回報的東西**。

## 做了什麼

`packages/engine/src/biometric/validation-log.ts` —— 三條檢查的純函式：
- **#15** PRV 閘門可達性：通過率**與分布**（0% 通過率在中位數 0.96 跟 0.70
  是兩件事：前者「門檻降一點」，後者「這個量在真手指上活不下來」）
- **#6** Pulse Lock 誠實性：「lock 出現但最終沒有讀數」必須是 0
- **#14** 跨天 SD ÷ 當日 SD（脈搏與脈搏節律各一份）

`/finger/` 開始前選情境（靜坐／邊走／手冰，**使用者說不是推測**），每次擷取
**接受與拒答都記**，證據層顯示報告 ＋「複製驗收報告」。報告**只有統計**，
沒有逐次資料列、沒有時間戳。

## 🔴 這一輪的教訓：斷言涵蓋哪條 path 是另一個獨立問題

兩條新斷言反向驗證時**都沒有變紅**：

1. 「拒答的擷取也要進紀錄」—— 我用的 fixture（lowPerfusion）走的是
   「analysed 但沒讀數」那條路，而那行記錄在**連分析都跑不動**的另一條
   `return` 裡。刪掉它，斷言照樣綠。
2. 「報告裡沒有時間戳」—— 只比對了 fixture 裡**第一筆**的 atMs。在報告標題印
   **最後**一筆就完全穿過去。

這是同形狀問題的第三次，但新意在於：**「條件對不對」跟「它跑到哪一條 code
path」是兩個獨立的問題，兩個都要問。** 已提煉進 PLAYBOOK。
修法：① 補一條直接餵空 frames 的案例 ② 斷言**模式**（`/\d{13}/`）不要斷言特定值。

## 下次接手點

- 🔴 **第 6/14/15 條現在可以跑了，但還沒有人跑。** founder 用手機走一趟
  （同一天 2 次以上、至少跨 2 天、其中至少 1 次邊走），按「複製驗收報告」
  貼回來，我就能判讀 —— 特別是 #15 的通過率會決定 PRV 門檻要不要調。
- §12 其餘 14 條仍需人工。
- 相機擷取層與 Breath Lock 擷取層仍未寫（需要 Mac）。
...
一份規則書：相機呼吸率只能以獨立的 **Breath Lock** 形式研究與釋出（十條硬規則）；
**camera-derived PRV 必須與 RR-derived HRV 分開，不得填進 HRV 欄位或被標成 HRV**；
兩個呼吸來源要比對、不一致就顯示衝突、**永不取平均**；以及一整段
「iPhone 量不到交感／副交感，不要做假的 SNS/PNS 分數，要做 Autonomic Regulation
Evidence」，含 `RegulationEvidence` 資料模型與 **phone-only 生理證據上限 15/100**。
收尾兩次「不確定就問我」。

## 我問了三題（因為第一題是真的衝突）

1. **Edge 接法** —— 那條硬規則「沒有可用生理訊號 ≠ 自動加高其他分項權重」
   跟**已經併進 main 的行為直接相反**。→ 選「證據上限＋中性錨點」。
2. **本輪範圍** → 選「契約與守門為止」。
3. **PRV 能不能顯示**（跟上一輪「相機 HRV 完全不顯示」衝突）→ 選「可顯示，
   但要自己的閘門」。

## 🔴 這一輪最重要的三個量測結果

**① 舊的 Edge 做法真的會因為資料變少而給高分。** 排除 driver 後重新正規化，
phone-only 讀數讓 `hr_stability` 從佔 15% 變成 30%（沒睡眠時 43%）。同一筆讀數
**有 HRV 時 72 分，拿掉 HRV 變 86 分**。現在有一條回歸測試鎖著，把舊行為放回去
它會紅並印出 72 → 86。

**② 品質分數擋不住 PRV 的誤差。** 感光雜訊完全不扣品質分（99 分）卻讓 PRV
錯 105–156%；重複性也擋不住（rep 0.4 配 177% 誤差）。唯一單調相關的是**拍形
穩定度**（每一拍對平均拍的相關），所以那才是 PRV 的閘門（0.97）。
⚠️ 這門檻是對合成訊號校的，真手指可能幾乎過不了 —— 那會是「PRV 實際上不出現」，
是誠實的結果不是 bug。實機第 15 條在驗。

**③ 我自己引進又抓到一個 bug**：移動量公式除以 50 而不是 100，把所有偏離量
放大一倍（加權平均 71.7 會算成 93.4）。既有測試沒抓到，因為它們的分數斷言
全是區間。是我在追一條一直紅的斷言時才浮出來的。

## 兩個「斷言在守一個不存在的東西」

- 回歸測試第一版用 hrBpm 53（「越低越好」），但 `hr_stability` 是**偏離基線**
  的量：68（= 基線）得 100 分、53 得 0 分。所以它在放大一個本來就 0 分的
  driver，把舊行為放回去照樣綠。改用 68 才真的會紅。
- cap 的斷言第一版拿兩筆讀數相減 —— sub-score 不是全部以錨點為中心，相減量到
  的是 **range**（兩倍 cap）不是貢獻。改成報出 `phoneEvidenceMovement` 再斷言。

## ⚠️ 我自己踩的坑

反向驗證後用 `git checkout --` 還原 sabotage，把同一個檔案裡**未 commit 的
修改一起洗掉**（to-reading.ts）。analyze.ts 我有用 `cp` 備份，這個檔沒有。
已提煉進 PLAYBOOK。

## 下次接手點

- 實機清單從 14 條變 **17 條**（新增：PRV 閘門在真手指上是否可達、Breath Lock
  對照參考來源、雙來源衝突）。仍然一條都沒跑。
- **Breath Lock 的擷取層沒寫**，`camera_breath_lock` 是關的。契約寫好了是為了
  讓它之後不可能被接成 pulse scan 的副產品。
- coupling(3) 與 regulation response(2) 的 5 分預算**保留未花**，有測試守著。
- REGULATION CHECK（90 秒三段含引導呼吸）founder 選了不做 —— 要做時記得那段
  文案需要過 compliance。
