...
不是「0 或 100 看起來怪」而已 —— **用經驗排名算百分位時，端點不是罕見，是必然**：
史上最好的那天**就是**最大值，n 天歷史裡有 2/n 的讀數壓在端點上。

改用 **z 分數 → 常態 CDF**：`score = clamp(round(100 × Φ(z)), 1, 99)`。
Φ(±2.33) 正好是 1% 與 99%，所以 1-99 是從數學掉出來的，不是剪出來的。

🔴 **最大的收穫不是端點，是 50 有了意義** —— 50 = 完全是你的常態。
絕對分 0-100 裡的 50 什麼都不代表。

## 二、三個一起處理的後果

1. **排名對象換掉**：舊 composite 含 `quality`（lighting/uniformity）= 房間不是人。
   拿它排名 → 房間暗一點分數就低。環境只能決定 confidence。
2. **門檻 70/40 → 80/20**：位置分對自己是均勻的，`0-39` 佔 40% 刻度
   → **永遠 40% 的日子落在最低帶**。壞產品 + 合規風險。
3. **樣本不足回 null**：`MIN_SAMPLES_FOR_SCORE = 14`，刻意高於引擎的 `ready`(5)。

## 三、選 z 分數而不是經驗排名的實作理由（查證過）

- `MetricBaseline` 只有 `{mean, std, sampleCount}` —— **沒有原始樣本**
- Welford + `MAX_SAMPLE_COUNT 100` / `DECAY_FACTOR 0.95` **本身就是軟性滾動窗口**，
  順便解掉「拿今天跟一個已經不存在的你比」
- 經驗排名要存時間序列，而**全 repo 沒有任何地方在存**

## 四、⚠️ 兩次抓到自己的斷言在說謊

1. **「Φ 再極端也取不到 0 或 1」是錯的** —— 浮點數上 |z|≈38 之後 underflow。
   保證的來源是 `Z_CLAMP`，不是曲線。註解與測試都改成講真話。
2. **樣本門檻那條是套套邏輯** —— 寫成 `MIN_SAMPLES_FOR_SCORE - 1`，
   把常數改成 1 之後**整組全綠**。改用字面值（5、13）釘住意圖才紅。
   🔴 **斷言引用被測常數 = 斷言跟著錯誤一起移動。**

## 五、下次接手點

- 165 tests 綠、`domain` tsc 綠。三組反向驗證都紅過。
- ⬜ **還沒接 UI、還沒持久化** —— 那四個數字（mean/std/n/ts）今天也還沒被存下來
- ⬜ `apps/mobile` 的 `deriveDailyEdgeScore`：fallback 是 `Math.sin(day)` 假分數、
  真路徑把 confidence 當分數（範疇錯誤），兩個都要換
- 🔴 尚未拍板：「高穩定度」對應到 brand.md 的 Above 還是 Below Baseline
